...
class T2035vPostTrainingTrajectoryAuditTests(unittest.TestCase):
    def test_early_residual_worsening_routes_full_path_interference(self) -> None:
        spec, permit, runtime, attempt, source, training_result, target, chunks = self._contract()
        result = build_result(
            spec=spec,
            permit=permit,
            ...
            target_chunk=target,
            normalized_target=source["normalized_padded_target"],
    ...
    def test_spec_permit_result_and_endpoint_drift_fail_closed(self) -> None:
        ...
        drift["mps_available"] = False
        with self.assertRaises(ValueError):
            verify_runtime_preflight(sign_payload(drift), spec=spec, permit=permit)
    ...
    def _contract(cls):
        ...
        source_result = sign_payload(
            {
                "schema_version": "fixture.t20_35q_result.v1",
                "normalized_padded_target": normalized_target,
                "normalized_padded_target_sha256": cls._hash(normalized_target),
                "new_trajectory_evaluations": source_rows,
                "path_interference_classification": "early_mid_path_interference",
                "t20_35o_result_identity_sha256": "1" * 64,
                "gate_b_passed": False,
                "selected_next_hypothesis": "design_full_path_self_consistency_correction",
            }
        )
        source_spec = sign_payload(
            {
                "schema_version": "fixture.t20_35q_spec.v1",
        ...
        training_run = sign_payload(
            {
                "schema_version": "fixture.t20_35t_run.v1",
        ...
        training_result = sign_payload(
            {
                "schema_version": "fixture.t20_35t_result.v1",
                "run_identity_sha256": training_run["identity_sha256"],
                "correction_objective_ratio_within_target": True,
                "objective_ratio_within_threshold": True,
                "all_decoded_chunks_within_threshold": False,
                "gate_b_passed": False,
                "selected_next_hypothesis": "balanced_objectives_pass_action_fail_route_post_training_path_audit",
        ...
        runtime_failure = sign_payload(
            {
                "schema_version": "fixture.t20_35u_runtime_failure.v1",
                "failure_phase": "lerobot_dataset_dependency_import_before_checkpoint_access",
                "missing_package": "datasets",
                "model_loaded": False,
                "checkpoint_read": False,
                "selected_next_hypothesis": "route_dependency_complete_runtime_preflight_before_distinct_path_audit",
            }
        )
        ...
        runtime = build_runtime_preflight(
            spec=spec,
            permit=permit,
            stack_identity=spec["lerobot_stack_identity_sha256"],
            dependency_versions={
                name: "1.0"
                for name in (
                    "datasets",
        ...
        return spec, permit, runtime, attempt, source_result, training_result, target, chunks
    ...
    def _new_trajectories(
        ...
        *,
        ...
        rows = []
        for source_row, chunk in zip(
            source["new_trajectory_evaluations"], chunks, strict=True
        ):
            steps = []
