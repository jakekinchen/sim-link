...
| 13o | **T20.36o — bounded X episode-0 bridge design/authority (in progress)** | Freeze a new PI0.5 queue execution contract at `n_action_steps=50`, one reset, starts 0/50/100/150/200, and executed lengths 50/50/50/50/44. Bind exact episode-0 windows, mask the six unexecuted tail actions, and extend X's correction/replay recipe with bounded updates and mid-run probes. Every executed action at every start must pass amendment `463477dc...` before Gate C execution can be requested. | Verified T20.36n; owner overnight priority 3; Brief 208 | Model load, optimizer/training, Gate C rollout, threshold change, new candidate, hardware, external compute, Brev until separate central authority/review |
