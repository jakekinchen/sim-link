...
| 13h | **T20.36h — exact SmolVLA Gate B attempt (pre-run verified)** | Implementation `29b9525`, preflight `b7e2938f...`, and permit `5fa7c1d2...` bind four raw checkpoint hashes, exact offline snapshots, canonical batch, MPS-only smoke, one-use runner, unchanged gate, and consumed-failure evidence. The sole attempt is next after remote confirmation. | Verified T20.36g design; Brief 199; Reviewer 250 | Second attempt, retry, sweep, policy selection, Gate B amendment, Gate C execution, hardware, external compute, Brev |
