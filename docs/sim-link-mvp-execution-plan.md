...
| 13n | **T20.36n — T20.35x tensor-only reproduction (pre-run verified)** | Retained X evidence has five hashes/aggregates but no decoded 50x6 tensors. Reproduce exactly the five registered seeds with two repeats, require all five source hashes, retain tensors directly, and score frozen amendment `463477dc...`. A pass routes only to separately authorized one-episode Gate C bridge work. | Verified-negative T20.36m; Brief 207; Reviewer 265; central decision `5b40131b...`; permit `2c6a1e78...` | Optimizer, training, new seeds/repeats, threshold change, Gate C execution, policy acceptance, hardware, network, external compute, Brev |
