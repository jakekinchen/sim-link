...
| 20b | **T20.43b — R1 ACT replacement rung (owner-authorized; next after T20.44 closeout origin)** | One replacement attempt per owner addendum `8b4a206`: stable T20.44 runtime plus real renderer smoke, unchanged fixed 10,000-update recipe lineage `b3a510f8...`, and fresh brief/Gate-A/composer/preflight/one-use/review ceremony. Supersedes T20.43's no-replacement exclusion exactly once. | Verified T20.44 terminal boundary on origin; recorded owner replacement authorization | Second replacement, recipe changes, correction objectives, threshold changes, hardware, network, external compute, Brev |
