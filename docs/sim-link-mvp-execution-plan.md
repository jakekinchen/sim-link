...
| 13o | **T20.36o — bounded X episode-0 bridge (baseline negative; optimizer authority next)** | Result `e6537428...` reproduces all start-zero and repeat hashes, retains 50 chunks/500 denoise records, and fails the frozen bridge: start 0 passes 3/5; all later starts pass 0/5 with 2,215 violations. Receipt `1154d524...` enables tracked-only rescore. Route only to separate bounded optimizer authority. | Verified T20.36n; owner overnight priority 3; Brief 208; Reviewers 267-271 | Optimizer/training before result remote preservation and separate authority, Gate C rollout, retry, threshold change, new candidate, hardware, external compute, Brev |
