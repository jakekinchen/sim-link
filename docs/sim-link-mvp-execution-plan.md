...
| 13i | **T20.36i — SmolVLA dependency-closure audit (active)** | Brief 200 resolves the pinned `smolvla` extra recursively against the consumed venv and signs a fail-closed correction design without installation or replacement authority. | Verified T20.36h dependency failure | Package install, environment mutation, replacement attempt, model access, Gate C, hardware, external compute, Brev |
