...
| B | One-batch memorization: the policy overfits a tiny fixed batch to near-zero action error | Model or trainer plumbing | T20.35x passed once; T20.36 failed retention. T20.36e ACT control passes its objective ratio but fails one physical maximum at 0.4425 rad; T20.36f localizes the exact joint/time and direct/queue path before SmolVLA or gate amendment |
...
| 13f | **T20.36f — ACT decode/per-joint localization (pre-run active)** | Reload only checkpoint `01b57134...`; reproduce final objective/action hash, compare direct chunk with queued decode, and localize normalized/physical error by joint and timestep before routing shared-path correction or SmolVLA entry. | Verified-negative T20.36e | Optimizer, retry, second training attempt, policy selection, gate change, Gate C, hardware, external compute, Brev |
...
not as a product-policy selection. T20.36e's negative result routes T20.36f
to identify the isolated physical maximum and verify direct-versus-queue decode
before any next policy track. SmolVLA may become the Mac-first VLA
