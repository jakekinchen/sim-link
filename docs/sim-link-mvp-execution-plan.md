...
| 13j | **T20.36j — corrected SmolVLA replacement attempt (live closure/processor gates pass; preflight artifact next)** | The exact four cached packages are installed; closure `ac8abed1...` passes; guarded AutoProcessor smoke `5223d8f0...` preserves the exact lexical snapshot, records six safe files, and observes no network/weights. Run corrected preflight `cb018b69...`; commit/push its one-use permit; then run at most one local-MPS replacement under unchanged Gate B. | Verified T20.36j-B; owner authorization 2026-07-16; Brief 203; Reviewers 255-258 | Network/extra packages; marker before preflight/remote permit; retry/sweep; gate change; Gate C before Gate B pass; hardware; external compute; Brev |
