...
| 13k | **T20.36k — consequence-calibrated Gate B amendment design (in progress)** | Use existing ACT/SmolVLA negative evidence plus model-free bounded joint perturbations to derive task-consequence sensitivity before any threshold. Keep strict uniform 0.05-rad evidence report-only; require a one-episode Gate C behavioral arbiter; candidate-result fitting fails closed. | Verified-negative T20.36e/T20.36f/T20.36j; owner pre-authorization; Brief 204 | ACT/SmolVLA retry, model load/inference, optimizer, immediate gate change, Gate C execution, policy selection, hardware, external compute, Brev |
