...
| 15 | **T20.38 — quantitative strict-v2 receipt contract (verified)** | Receipt `02268a1a...` binds 33 raw/normalized signed margins, actor/evidence guards, hard conjunction, source identities, and a deterministic zero-headroom grasp-contact bottleneck. It agrees with analytic source success while withholding policy/MuJoCo/physical claims. | T20.36o terminal negative; Brief 209; Reviewer 278; implementation `f9c3682` | Model load/inference, optimizer, gate change, history rewrite, Gate C execution, policy selection, hardware, external compute, Brev |
| 16 | **T20.39 — counterexample archive schema/bootstrap (verified)** | Receipt `8277b09f...` and index `05908b6d...` seed one truthful evidence-only T20.19 source-controller boundary negative. Full trace bytes are not remotely retained, so replay/training/policy-blame remain denied. | T20.19; verified T20.38; Brief 210; Reviewer 279; implementation `80d2992` | Scene search, policy blame without expert competence, replay gate activation, training ingestion, hardware, external compute, Brev |
...
| 18 | **T20.41 — owner capability-route decision (blocked)** | Select and authorize the next capability strategy after ACT, SmolVLA, and X all failed forward routing. Must start a fresh brief; cannot silently resume the optimizer alphabet. | T20.36o terminal negative; verified T20.39; owner decision | New architecture, optimizer, gate change, Gate C execution, hardware, external compute, or Brev without fresh authority |
