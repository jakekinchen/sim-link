...
| 19 | **T20.42 — R0 dataset expansion by construction (active; Briefs 216-218)** | Order 64–128 new strict-v2-success scripted episodes over sanctioned cube-pose/init variation (plus cheap recovery branches), compiled through existing raw-store→compiler→window→LeRobotDataset contracts; MEAN_STD from the training split only; frozen held-out seeds 6–7 plus one fresh pose band; one signed dataset/statistics/mixture manifest. Reviewers 286/287 accept the 119+9 construction/authority contracts; Reviewer 288 accepts the `d24ad0e`/`c435809` collector, exclusive materializer, fixed runner, signed pre-run gate, and success/terminal-negative evidence routes. Compact authority materialization is next, followed by Reviewer 289 before the marker. | Recorded T20.41 route; verified scripted expert and randomization contracts; Briefs 216-218; Reviewers 286-288; fresh bounded owner window | Physics-parameter randomization, new subsystems, model/optimizer work, marker/generation before Reviewer 289, retry, hardware, external compute, Brev |
