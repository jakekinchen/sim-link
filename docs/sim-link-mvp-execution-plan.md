...
| 12b | **T20.35.x — conditional Gate B discriminators** | T20.35c's expert-only ceiling passes the objective gate but misses action error; T20.35d-f localize 77.87% of errors to wrist roll/gripper and systematic normalized bias without clipping. T20.35g reproduces the 10-step baseline exactly and rejects 20/50-step cadence because both worsen worst and mean error. Route T20.35h to a model-free leave-one-seed-out output-bias correction ceiling before any model or optimizer correction. | Verified-negative T20.35; corrected coverage audit `446a0686...`; T20.35c-g signed evidence; reviewer chain through Decision 210 | Combined-factor changes, unreviewed sweeps, second batch, Gate C changes, promotion |
