...
| 12b | **T20.35.x — conditional Gate B discriminators** | T20.35b corrects the initial generic audit against pinned PI0.5 source: there is intentionally no state projection; the real uncovered modules are `time_mlp_in/out`, missed because the default PEFT regex uses stale `action_time_mlp` names. Route next to one no-LoRA expert-only unfreeze capacity ceiling before deferred LR/budget rungs; separately calibrate the attainable objective floor if coverage is corrected. | Verified-negative T20.35; T20.35a tensor enumeration; corrected audit `446a0686...`; Reviewer Decision 201 | Combined-factor changes, unreviewed sweeps, second batch, Gate C changes, promotion |
