...
| 19 | **T20.42 — R0 dataset expansion by construction** | Order 64–128 new strict-v2-success scripted episodes over sanctioned cube-pose/init variation (plus cheap recovery branches), compiled through existing raw-store→compiler→window→LeRobotDataset contracts; MEAN_STD from the training split only; frozen held-out seeds 6–7 plus one fresh pose band; one signed dataset/statistics/mixture manifest. | Recorded T20.41 route; verified scripted expert and randomization contracts | Physics-parameter randomization, new subsystems, model/optimizer work, hardware, external compute, Brev |
