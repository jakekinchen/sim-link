...
| 13f | **T20.36f — ACT decode/per-joint localization (pre-run verified)** | Implementation `489be59`, preflight `40e24b5e...`, and inference-only permit `d9daae7b...` bind checkpoint `01b57134...`. After remote evidence preservation, reproduce the final objective/hash, compare direct and queued decode, and localize joint/time error. | Verified-negative T20.36e | Optimizer, retry, second training attempt, policy selection, gate change, Gate C, hardware, external compute, Brev |
