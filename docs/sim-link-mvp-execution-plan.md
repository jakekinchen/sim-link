...
| 19 | **T20.42 — R0 dataset expansion by construction (active; Briefs 216-218)** | Order 64–128 new strict-v2-success scripted episodes over sanctioned cube-pose/init variation (plus cheap recovery branches), compiled through existing raw-store→compiler→window→LeRobotDataset contracts; MEAN_STD from the training split only; frozen held-out seeds 6–7 plus one fresh pose band; one signed dataset/statistics/mixture manifest. Reviewers 286-288 accept the construction, authority contracts, and `d24ad0e`/`c435809` fixed runner. Authority commit `93d7708` and Reviewer 289 acceptance `97694eb5...` open exactly one fixed 119+9 attempt after the acceptance is origin-preserved. | Recorded T20.41 route; verified scripted expert and randomization contracts; Briefs 216-218; Reviewers 286-289; fresh bounded owner window | Physics-parameter randomization, new subsystems, model/optimizer work, execution before acceptance origin preservation, retry, hardware, external compute, Brev |
