...
| 13e | **T20.36e — exact ACT control pre-run and execution (pre-run verified)** | Implementation `0f0caa7`, central decision `9c2a16d2...`, model-free preflight `d29b93e7...`, and one-use permit `75f5e163...` bind the exact runner/runtime/schedule. Execute the sole local-MPS attempt only after this evidence boundary reaches origin. | Verified T20.36d design | Any pre-preservation attempt/model/optimizer, retry, sweep, policy selection, Gate B amendment, Gate C, hardware, external compute, Brev |
...
T20.36e's pre-run boundary is now reviewed and may execute only after its
signed evidence is remotely preserved. SmolVLA may become the Mac-first VLA
