...
| 13j | **T20.36j — corrected SmolVLA replacement attempt (offline correction complete; preflight next)** | The exact four cached packages are installed offline; collector correction `9e9272c` handles byte-identical editable `PKG-INFO` and wheel `METADATA`, and live 57-package closure `ac8abed1...` passes. Run corrected preflight `cb018b69...`; commit/push its one-use permit; then run at most one local-MPS replacement under unchanged Gate B. | Verified T20.36j-B; owner authorization 2026-07-16; Brief 203; Reviewers 255-256 | Network/extra packages; marker before preflight/remote permit; retry/sweep; gate change; Gate C before Gate B pass; hardware; external compute; Brev |
