...
| 12b | **T20.35.x — conditional Gate B discriminators** | T20.35a verified an actual PEFT coverage fault: all 36 expert q/v pairs and action in/out are saved, but state projection and both action-time MLP projections are absent. Route next to one no-LoRA expert-only unfreeze capacity ceiling before the deferred 2.5e-4/500-update and 5,000-update rungs; separately calibrate the attainable objective floor if coverage is corrected. | Verified-negative T20.35; verified T20.35a coverage audit `d1109ae8...`; Reviewer Decision 200 | Combined-factor changes, unreviewed sweeps, second batch, Gate C changes, promotion |
