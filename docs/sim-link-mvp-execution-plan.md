...
| 19 | **T20.42 — R0 dataset expansion by construction (active; Briefs 216/217)** | Order 64–128 new strict-v2-success scripted episodes over sanctioned cube-pose/init variation (plus cheap recovery branches), compiled through existing raw-store→compiler→window→LeRobotDataset contracts; MEAN_STD from the training split only; frozen held-out seeds 6–7 plus one fresh pose band; one signed dataset/statistics/mixture manifest. Reviewer 286 accepts implementation `23cb5fb`: 119 candidates + 9 fresh held out are frozen. Reviewer 287 accepts the `c2a4512`/`9530671` construction-only central/preflight/permit/marker contracts; no authority artifact exists and generation remains false pending a fresh brief, active owner window, materialization, and pre-run review. | Recorded T20.41 route; verified scripted expert and randomization contracts; Briefs 216/217; Reviewers 286/287 | Physics-parameter randomization, new subsystems, model/optimizer work, authority materialization or generation before a fresh active owner window and pre-run review, hardware, external compute, Brev |
