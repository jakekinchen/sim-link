...
| B | One-batch memorization: the policy overfits a tiny fixed batch to near-zero action error | Model or trainer plumbing | T20.36g freezes exact offline SmolVLA entry `fb217f3e...`; T20.36h implements and separately authorizes its one-use unchanged-gate attempt |
...
| 13n | **T20.36n — T20.35x tensor-only reproduction (in progress)** | Retained X evidence has five hashes/aggregates but no decoded 50x6 tensors. Reproduce exactly the five registered seeds with two repeats, require all five source hashes, retain tensors directly, and score frozen amendment `463477dc...`. A pass routes only to separately authorized one-episode Gate C bridge work. | Verified-negative T20.36m; owner overnight priority 2; Brief 207; fresh central inference authority required | Optimizer, training, new seeds/repeats, threshold change, Gate C execution, policy acceptance, hardware, network, external compute, Brev |
