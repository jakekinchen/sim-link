...
| 13j | **T20.36j — corrected SmolVLA replacement attempt (owner-authorized, in progress)** | Install only the four distributions bound by cache manifest `6cc7235c...`, offline; execute corrected preflight contract `cb018b69...`; remotely verify a fresh central decision and one-use permit; then run at most one local-MPS replacement attempt under unchanged Gate B. | Verified T20.36j-B; owner authorization 2026-07-16; Brief 203 | Network/extra packages; marker before preflight/remote permit; retry/sweep; gate change; Gate C before Gate B pass; hardware; external compute; Brev |
