...
| 19 | **T20.42 — R0 dataset expansion by construction (active, Brief 216)** | Order 64–128 new strict-v2-success scripted episodes over sanctioned cube-pose/init variation (plus cheap recovery branches), compiled through existing raw-store→compiler→window→LeRobotDataset contracts; MEAN_STD from the training split only; frozen held-out seeds 6–7 plus one fresh pose band; one signed dataset/statistics/mixture manifest. Brief 216 opens tests-first contract implementation; generation remains separately gated by reviewed central/preflight/one-use authority. | Recorded T20.41 route; verified scripted expert and randomization contracts; Brief 216 | Physics-parameter randomization, new subsystems, model/optimizer work, hardware, external compute, Brev |
