...
| 13j | **T20.36j — corrected SmolVLA replacement attempt (implementation verified; offline correction next)** | Implementation `b35acd1` and central decision `aa0992e6...` are remote. Install only the four distributions bound by cache manifest `6cc7235c...`, offline; execute corrected preflight `cb018b69...`; commit/push its fresh one-use permit; then run at most one local-MPS replacement attempt under unchanged Gate B. | Verified T20.36j-B; owner authorization 2026-07-16; Brief 203; Reviewer 255 | Network/extra packages; marker before preflight/remote permit; retry/sweep; gate change; Gate C before Gate B pass; hardware; external compute; Brev |
