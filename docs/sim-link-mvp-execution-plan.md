...
| 13j-B | **T20.36j-B — corrected replacement-preflight contract (in progress)** | Implement recursive installed-closure validation and make offline AutoProcessor construction a mandatory pre-marker gate while full policy construction remains counted. | Verified T20.36j-A; Brief 202 | Package install, live AutoProcessor/model access, marker, replacement execution, Gate B change, Gate C, hardware, external compute, Brev |
| 13j | **T20.36j — corrected SmolVLA replacement attempt (blocked)** | Requires new owner authority to install only the four distributions bound by cache manifest `6cc7235c...`, offline, and authorize at most one replacement local-MPS attempt after a signed environment manifest and offline AutoProcessor smoke. | Verified T20.36j-A plus new owner decision | Any mutation or attempt before owner decision; retry/sweep; gate change; Gate C; hardware; external compute; Brev |
