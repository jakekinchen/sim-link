...
| Robo Scan integration | Treat the modular scan/scene/calibration repository as an explicit immutable artifact source, not a runtime dependency; preserve its local authority boundary and deduplicate only after verified handoffs. | [Robo Scan integration boundary](./robo-scan-integration.md), [integration roadmap](./robo-scan-sim-link-integration-roadmap.md), and [Brief 144](./briefs/144-robo-scan-sim-link-integration-roadmap.md) |
| Workcell foundry MVP cut | Build the clean PI0.5 sim policy first; pull forward state forks, a discrete ensemble, observer roles, paired traces, and timing; defer posterior machinery and cut RL/world-model/skill-graph/scientist infrastructure until evidence creates a need. | [Sim-link MVP execution plan](./sim-link-mvp-execution-plan.md) and [Brief 146](./briefs/146-sim-link-mvp-cut-and-local-task-queue.md) |
