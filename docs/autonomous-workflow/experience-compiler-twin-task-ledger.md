...
run_state: T20.17 in progress under Brief 137: owner-requested bespoke-versus-package recreation consolidation; retain proof-bearing bespoke contracts, remove only unreferenced test-only duplication, and establish a LeRobot-native signed-manifest/actual-pipeline boundary before clean-base training
current_milestone: M20 simulation-only clean supervision
current_task: T20.17 in progress: establish the package-boundary map and first safe consolidation, then bind clean pi05_base initialization to a single LeRobotDataset's statistics and actual processor outputs before any new training run
