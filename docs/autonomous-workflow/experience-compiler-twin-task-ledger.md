...
run_state: T20.36b spec 6e56f6ff... and decision e709c30c... are remotely preserved at 5fdc36c; only X retains Gate B, no coverage candidate exists, and Brief 194 opens a read-only local ACT/SmolVLA preflight.
current_milestone: M20 simulation-only clean supervision
current_task: T20.36c in progress under Brief 194; inventory local ACT/SmolVLA source, cache, and canonical-dataset compatibility without downloads, checkpoint reads, model load, inference, optimizer, or policy selection
...
next_step: implement and verify the read-only T20.36c local policy-track preflight; any model-track selection or run remains an owner/reviewer decision after that evidence
```
