...
run_state: T20.17 in progress under Brief 137; first boundary `af317bc` reviewed by Decision 167 retires only the three repository-unreferenced test-only modules; next establish a LeRobot-native signed-manifest/actual-pipeline boundary before clean-base training
current_milestone: M20 simulation-only clean supervision
current_task: T20.17 in progress: the package-boundary map and first safe retirement are verified; now bind clean pi05_base initialization to a single LeRobotDataset's statistics and actual processor outputs before any new training run
