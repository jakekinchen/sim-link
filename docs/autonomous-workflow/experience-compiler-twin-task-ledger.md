...
run_state: T20.17 in progress under Brief 138; first boundary `af317bc` reviewed by Decision 167 retires only the three repository-unreferenced test-only modules; active slice builds the thin LeRobot-native signed episode manifest before actual-pipeline observation and clean-base training
current_milestone: M20 simulation-only clean supervision
current_task: T20.17 in progress: the package-boundary map and first safe retirement are verified; now bind episode-level provenance, eligibility, and quarantine to one actual LeRobotDataset without creating a second training store
