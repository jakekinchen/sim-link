...
current_task: T20.17 in progress under Brief 147: persistently materialize and verify the six-episode source-native LeRobot training dataset, bind held-out seeds 6-7 and the complete local pi05_base snapshot, then compose T20.17-specific simulation-only authority before any optimizer execution
...
next_step: execute Brief 147 source-native clean-base preflight; optimizer/model execution remains denied until its implementation, tests, review, commit, remote preservation, and central authority agree
```
