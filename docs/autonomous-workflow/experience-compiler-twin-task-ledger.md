...
run_state: T20.7 in progress under Brief 127; fresh equal-sample four-model canary gates the fixed-semantics local-MPS bake-off
...
next_step: freeze the common source-start plan and prove finite local load/preprocess/forward/backward for PI0.5, SmolVLA, ACT, and Diffusion Policy
```
...
| T20.7 | in_progress | T20.1, T20.6 | Bake off PI0.5, SmolVLA, ACT, and Diffusion Policy | Brief 127; fresh equal-sample local-MPS rung, gated by finite four-model canary; same semantics, splits, samples, seeds, and proof modes |
