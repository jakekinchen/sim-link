...
run_state: T20.17 in progress under Brief 140; first boundary `af317bc` reviewed by Decision 167 retires only the three repository-unreferenced test-only modules; second boundary `1f5154a` reviewed by Decision 168 binds actual LeRobotDataset episode provenance/eligibility/quarantine without a parallel store; third boundary `cf1d05d` reviewed by Decision 169 invokes the actual pinned PI0.5 processor offline and signs its finite outputs; active fourth slice extracts the live constructive geometry primitive before retiring the eight T19 wrappers
current_milestone: M20 simulation-only clean supervision
current_task: T20.17 in progress: the package-boundary map, first safe retirement, actual-dataset provenance view, and actual pinned PI0.5 processor observation are verified; now replace the T19 wrapper/search dependency with a named live constructive geometry primitive while preserving signed historical diagnostics
