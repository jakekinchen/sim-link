...
current_task: T20.17 in progress after Brief 147: open a new exact bounded-execution brief before loading the clean base or starting the fixed local-MPS optimizer campaign
...
next_step: open the bounded T20.17 clean-base campaign brief, reverify live authority, then run only its fixed local-MPS training and held-out seed-6 strict evaluation
```
