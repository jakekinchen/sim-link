...
run_state: T20.17 in progress under Brief 139; first boundary `af317bc` reviewed by Decision 167 retires only the three repository-unreferenced test-only modules; second boundary `1f5154a` reviewed by Decision 168 binds actual LeRobotDataset episode provenance/eligibility/quarantine without a parallel store; active slice invokes and hashes actual pinned processor outputs
current_milestone: M20 simulation-only clean supervision
current_task: T20.17 in progress: the package-boundary map, first safe retirement, and actual-dataset provenance view are verified; now invoke and hash actual pinned PI0.5 processor outputs without a shadow preprocessing implementation
