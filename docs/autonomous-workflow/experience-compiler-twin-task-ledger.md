...
run_state: T20.15 verified through Brief 135/Reviewer 165; action postprocessing dominates the arm regression by 3.68x over the runner-up; T20.16 is next eligible
current_milestone: M20 simulation-only clean supervision
current_task: T20.16 pending: test checkpoint state/arm scaling with dataset-derived gripper-only unnormalization at frame zero and in one fixed seed-2 strict rendered rollout
...
| T20.16 | pending | T20.15 | Test checkpoint arm/state scaling with dataset-derived gripper-only action postprocessing | No training; exact frame-zero preflight then fixed seed-2 strict rollout with five keyframes and margins |
