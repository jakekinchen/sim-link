...
run_state: T20.16 is in progress under Brief 136; checkpoint state/arm scaling plus dataset-derived gripper-only action postprocessing and one fixed seed-2 strict rendered rollout
current_milestone: M20 simulation-only clean supervision
current_task: T20.16 in progress: test checkpoint state/arm scaling with dataset-derived gripper-only unnormalization at frame zero and in one fixed seed-2 strict rendered rollout
...
next_step: retain the checkpoint state preprocessor and arm action statistics, substitute only dataset-derived gripper action statistics, prove the frame-zero error decomposition, then run one fixed seed-2 strict rollout with five keyframes
```
...
| T20.16 | in_progress | T20.15 | Test checkpoint arm/state scaling with dataset-derived gripper-only action postprocessing | Brief 136; no training; exact frame-zero preflight then fixed seed-2 strict rollout with five keyframes and margins |
