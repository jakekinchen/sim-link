...
run_state: T20.25 is verified through Brief 156 and Reviewer Decision 186: recovery improves sampled pre-contact action MAE but regresses frame-zero MAE; the clean prior action hash is not cross-process reproducible, so training attribution remains blocked.
current_milestone: M20 simulation-only clean supervision
current_task: T20.25 verified diagnostic; no policy accepted; next safe task is repeated frozen-inference variability characterization
...
next_step: quantify repeated within-candidate frozen-inference variability before any clean-versus-recovery training attribution or optimizer allocation
```
