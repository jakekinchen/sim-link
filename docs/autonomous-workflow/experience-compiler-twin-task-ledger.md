...
run_state: T20.17 in progress under Brief 142; first boundary `af317bc` reviewed by Decision 167 retires only the three repository-unreferenced test-only modules; second boundary `1f5154a` reviewed by Decision 168 binds actual LeRobotDataset episode provenance/eligibility/quarantine without a parallel store; third boundary `cf1d05d` reviewed by Decision 169 invokes the actual pinned PI0.5 processor offline and signs its finite outputs; fourth boundary `1aceebe` reviewed by Decision 170 extracts the live constructive geometry primitive and retires the eight T19 wrappers; fifth boundary `61f2d8c` reviewed by Decision 171 specifies the minimal future live adapter without touching physical evidence; active sixth slice establishes the project documentation front door and explainer map
current_milestone: M20 simulation-only clean supervision
current_task: T20.17 in progress: the requested bespoke-versus-package recreation map is verified (compact bespoke boundaries, native LeRobot provenance and processor observation, retired T19 wrappers, and live-adapter invariants); now consolidate the navigation/explainer layer without duplicating the canonical state or starting the separate fresh source-backed clean-base campaign
