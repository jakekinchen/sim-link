...
run_state: T20.7 in progress under Brief 127/Reviewer 156; hardened four-model canary passed and authorizes only the exact common 20-sample rung
current_milestone: M20 simulation-only clean supervision
current_task: T20.7 in progress under Brief 127: run a fail-closed common-sample canary before any four-model ranking
...
next_step: implement the exact 20-update/20-sample local-MPS rung for all four canary-qualified models, then compare only fixed-seed policy-owned closed-loop strict behavior
```
...
| T20.7 | in_progress | T20.1, T20.6 | Bake off PI0.5, SmolVLA, ACT, and Diffusion Policy | Brief 127/Reviewer 156 interim; run 003 proves finite same-sample forward/backward for all four with full source/runtime binding; exact 20-sample rung next |
