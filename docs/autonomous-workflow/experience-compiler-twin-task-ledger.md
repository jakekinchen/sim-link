...
| T19.1 | in_progress | M16, read authority | Run read-only servo/firmware/register census | Brief 212; formal same-thread runtime, central read-only decision, one-use permit, remote open-gate boundary, and immutable zero-write/no-motion snapshot required before completion |
