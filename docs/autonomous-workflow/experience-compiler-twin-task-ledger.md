...
run_state: T20.17 remains in progress; Brief 146 is verified at `81c9a25` and reviewed by Decision 176. The accepted MVP cut restores clean pi05_base dataset-native training as the immediate target, records T20.18-T20.22 as pending only, and leaves Robo Scan I4 as a separate external dependency.
current_milestone: M20 simulation-only clean supervision
current_task: T20.17 in progress with no active implementation brief: open one precise clean pi05_base dataset-native training and strict-v2 evaluation brief next; T20.18-T20.22 remain dependency-ordered pending sim-link tasks; Robo Scan I4 remains a separate external dependency
