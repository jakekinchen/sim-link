...
| T19.1 | in_progress | M16, read authority | Run read-only servo/firmware/register census | Brief 212 implementation `288e938` remote; Reviewer 281 opens exactly one serial-only 54-read/no-retry session after the transition is remote; formal same-thread runtime, central decision, one-use permit, and immutable zero-write/no-motion snapshot still required |
