...
run_state: T20.18 is verified through Brief 149 and Reviewer Decision 179. T20.19 is active under Brief 150 with one fixed 12-cell, one-factor-only recovery-controller ensemble.
current_milestone: M20 simulation-only clean supervision
current_task: T20.19 in progress under Brief 150: exact-parent same-seed 12-cell pose/friction/timing/gripper recovery ensemble
...
next_step: tests-first immutable cell manifest and deterministic scorecard; do not infer a calibrated posterior or start optimizer work
```
