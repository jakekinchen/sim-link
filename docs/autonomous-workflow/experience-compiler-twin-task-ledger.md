...
run_state: T20.17 remains in progress; Brief 147 is verified at `b826e3f` and reviewed by Decision 177. The persistent six-episode source-native dataset, held-out seeds 6-7, complete pi05_base cache, fixed campaign, and simulation-training readiness decision are verified.
current_milestone: M20 simulation-only clean supervision
current_task: T20.17 in progress under Brief 148: implement and run only the exact official-LeRobot rank-4 local-MPS 250-update campaign, then evaluate the final candidate unassisted on frozen seed 6
...
next_step: tests-first implementation of the Brief 148 runner and evidence contract, live authority verification, fixed local training, then held-out seed-6 strict evaluation
```
