...
run_state: T20.8 verified through Brief 128/Reviewer 158 as a central fail-closed policy rejection; T20.9 is next eligible
current_milestone: M20 simulation-only clean supervision
current_task: T20.9 pending: test the recorded held-out expert actions through the exact policy closed-loop adapter and localize the first source-versus-execution divergence
...
next_step: write Brief 129 for the expert-action oracle rollout audit and prove whether the exact execution path can reproduce the recorded strict grasp
```
...
| T20.9 | pending | T20.8 | Replay held-out source expert actions through the exact policy closed-loop adapter and localize first divergence | Same reset, action application, limits, evaluator, five keyframes, and measured margins as T20.7; no optimizer run |
