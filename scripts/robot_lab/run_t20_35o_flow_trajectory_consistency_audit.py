#!/usr/bin/env python3
"""Run or verify the one T20.35o flow-trajectory consistency audit."""
...
from scenesmith.robot_lab.t20_35o_flow_trajectory_consistency_audit import (  # noqa: E402
...
def main() -> int:
    ...
    if spec.get("required_python_major_minor") != [3, 12] or list(
        ...
        raise RuntimeError("T20.35o requires the reviewed Python 3.12 runtime")
    ...
    if (
        ...
        raise ValueError("T20.35o inherited authority identity drifted")
    if _file_tree(REPO_ROOT / CHECKPOINT_ROOT) != spec["checkpoint_tree"]:
        raise ValueError("T20.35o checkpoint tree drifted before evaluation")
    if hashlib.sha256(
        (REPO_ROOT / SAMPLER_SOURCE_PATH).read_bytes()
    ).hexdigest() != spec["sampler_source_sha256"]:
        raise ValueError("T20.35o sampler source drifted before evaluation")
    ...
    if (REPO_ROOT / ATTEMPT_PATH).exists() or (REPO_ROOT / RESULT_PATH).exists():
        raise FileExistsError("T20.35o evaluation already exists; use --verify")
    ...
    if stack["identity_sha256"] != spec["lerobot_stack_identity_sha256"]:
        raise ValueError("T20.35o live LeRobot stack drifted")
    import torch
    ...
    if not torch.backends.mps.is_available():
        raise RuntimeError("T20.35o requires the authorized local MPS runtime")
    ...
    if sorted(checkpoint) != sorted(checkpoint_config["trainable_parameter_names"]):
        raise ValueError("T20.35o checkpoint key set drifted")
    for row in checkpoint_config["trainable_tensor_manifest"]:
        ...
        if (
            ...
            raise ValueError("T20.35o checkpoint tensor drifted or is non-finite")
    ...
    if hashlib.sha256(
        ...
        raise ValueError("T20.35o dataset target drifted")
    ...
    if hasattr(policy, "peft_config") or "Peft" in type(policy).__name__:
        raise ValueError("T20.35o unexpectedly constructed a PEFT policy")
    if (
        ...
        raise ValueError("T20.35o cadence or action width drifted")
    ...
    if (
        ...
        raise ValueError("T20.35o model parameter boundary drifted")
    ...
    normalized_target_tensor = policy.prepare_action(training_batch)
    if list(normalized_target_tensor.shape) != [1, 50, MAXIMUM_ACTION_DIMENSIONS]:
        raise ValueError("T20.35o normalized padded target shape drifted")
    normalized_target = (
        normalized_target_tensor[0].detach().cpu().float().numpy().astype(float).tolist()
    )
    ...
    with torch.no_grad():
        for seed_index, seed in enumerate(INFERENCE_SEEDS):
            ...
            if base_noise_sha != spec["base_noise_sha256_by_seed"][seed_index]:
                raise ValueError("T20.35o base noise failed exact reproduction")
            captured = []
            original_denoise_step = policy.model.denoise_step

            def capture_denoise_step(*args, **kwargs):
                state = kwargs.get("x_t")
                timestep = kwargs.get("timestep")
                if state is None or timestep is None:
                    raise ValueError("T20.35o denoise-step call signature drifted")
                ...
                if abs(observed_time - expected_time) > 1e-6:
                    raise ValueError("T20.35o live denoise-step time grid drifted")
            ...
            policy.model.denoise_step = capture_denoise_step
            try:
                actions = policy.predict_action_chunk(
                    processed, noise=base_noise * mask
                )
            finally:
                policy.model.denoise_step = original_denoise_step
            if len(captured) != NUM_INFERENCE_STEPS:
                raise ValueError("T20.35o did not capture exactly ten denoise steps")
            if list(actions.shape) != [1, 50, ACTIVE_ACTION_DIMENSIONS]:
                raise ValueError("T20.35o predicted action shape drifted")
            decoded = []
