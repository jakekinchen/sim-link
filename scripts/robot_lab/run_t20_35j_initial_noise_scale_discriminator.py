#!/usr/bin/env python3
"""Run or verify the one T20.35j initial-noise-scale evaluation."""
...
def main() -> int:
    ...
    if spec.get("required_python_major_minor") != [3, 12] or list(
        ...
        raise RuntimeError("T20.35j requires the reviewed Python 3.12 runtime")
    ...
    if (
        ...
        raise ValueError("T20.35j inherited authority identity drifted")
    if _file_tree(REPO_ROOT / CHECKPOINT_ROOT) != spec["checkpoint_tree"]:
        raise ValueError("T20.35j checkpoint tree drifted before evaluation")
    if hashlib.sha256((REPO_ROOT / SAMPLER_SOURCE_PATH).read_bytes()).hexdigest() != spec[
        "sampler_source_sha256"
    ]:
        raise ValueError("T20.35j sampler source drifted before evaluation")
    ...
    if (REPO_ROOT / ATTEMPT_PATH).exists() or (REPO_ROOT / RESULT_PATH).exists():
        raise FileExistsError("T20.35j evaluation already exists; use --verify")
    ...
    if stack["identity_sha256"] != spec["lerobot_stack_identity_sha256"]:
        raise ValueError("T20.35j live LeRobot stack drifted")
    import torch
    ...
    if not torch.backends.mps.is_available():
        raise RuntimeError("T20.35j requires the authorized local MPS runtime")
    ...
    if sorted(checkpoint) != sorted(checkpoint_config["trainable_parameter_names"]):
        raise ValueError("T20.35j checkpoint key set drifted")
    for row in checkpoint_config["trainable_tensor_manifest"]:
        ...
        if (
            ...
            raise ValueError("T20.35j checkpoint tensor drifted or is non-finite")
    ...
    if hashlib.sha256(
        ...
        raise ValueError("T20.35j dataset target drifted")
    ...
    if hasattr(policy, "peft_config") or "Peft" in type(policy).__name__:
        raise ValueError("T20.35j unexpectedly constructed a PEFT policy")
    if policy.model.config.num_inference_steps != NUM_INFERENCE_STEPS:
        raise ValueError("T20.35j denoising cadence drifted")
    ...
    if (
        ...
        raise ValueError("T20.35j model parameter boundary drifted")
...
def _decoded_scales(policy, preprocessor, postprocessor, raw_batch, torch):
    observation = {
    ...
    policy.eval()
    with torch.no_grad():
        for scale in INITIAL_NOISE_SCALES:
            rows = []
            for seed in INFERENCE_SEEDS:
                ...
                actions = policy.predict_action_chunk(
                    processed, noise=base_noise * scale
                )
                if list(actions.shape) != [1, 50, 6]:
                    raise ValueError("T20.35j predicted action shape drifted")
                decoded = []
