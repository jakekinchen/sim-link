#!/usr/bin/env python3
"""Evaluate the frozen T20.24 adapter unassisted on held-out seeds 6 and 7."""
...
ACTION_HORIZON = 5
SCHEMA_VERSION = "scenesmith.t20_24_recovery_augmented_closed_loop.v1"
...
def main() -> int:
    ...
    if any(path.exists() for path in outputs.values()):
        raise FileExistsError("T20.24 held-out evaluation output already exists")
    stack = activate_lerobot_stack(repo_root=REPO_ROOT, stage="inference")
    require_active_t20_23_authority(repo_root=REPO_ROOT)
    run_root = REPO_ROOT / RUN_ROOT
    summary_path = run_root / RUN_SUMMARY_PATH
    summary = load_strict_json(summary_path)
    verify_signed_payload(summary, label="T20.24 recovery-augmented training run")
    ...
    if any(row["outcome"]["strict_success"] is not True for row in source_entries.values()):
        raise ValueError("T20.24 held-out source episode is not strict-success evidence")
    ...
    if not torch.backends.mps.is_available():
        raise RuntimeError("T20.24 held-out evaluation requires local MPS")
    config = PreTrainedConfig.from_pretrained(checkpoint, local_files_only=True)
    snapshot = Path(config.pretrained_path).resolve()
    if snapshot.name != EXPECTED_MODEL_REVISION or not (
        snapshot / "model.safetensors"
    ).is_file():
        raise ValueError("T20.24 evaluation base snapshot binding drifted")
    config.device = "mps"
    ...
    if Path(peft_config.base_model_name_or_path).resolve() != snapshot:
        raise ValueError("T20.24 adapter base-model binding drifted")
    ...
    for seed in HELD_OUT_SEEDS:
        ...
        def policy_action(images: dict[str, np.ndarray], state: np.ndarray) -> np.ndarray:
            ...
            if values.shape != (6,) or not np.isfinite(values).all():
                raise ValueError("T20.24 candidate emitted an invalid action")
        ...
        rollout = run_policy_grasp_closed_loop(
            ...
            task_id="T20.24",
            evidence_mode=(
                f"recovery_augmented_pi05_held_out_seed_{seed}_unassisted"
            ),
        ...
        if rollout.get("seed") != seed:
            raise ValueError("T20.24 rollout seed binding drifted")
        if any(
            rollout.get(key) != 0
            for key in ("projected_action_frame_count", "active_assist_frame_count")
        ):
            raise ValueError("T20.24 rollout used projection or assistance")
...
def _verify_checkpoint(checkpoint: Path, summary: dict[str, Any]) -> None:
    for row in summary["checkpoint_tree"]:
        ...
        if (
            ...
            raise ValueError(f"T20.24 checkpoint file drifted: {row['path']}")
