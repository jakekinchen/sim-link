#!/usr/bin/env python3
"""Write or verify the jointly aligned wrist grasp search."""
...
OUTPUT = REPO_ROOT / "configurations/robot_lab/horizontal_axis_grasp_search.json"
...
def main() -> int:
    ...
    payload = build_horizontal_axis_grasp_search()
    if args.verify:
        if load_strict_json(OUTPUT) != payload:
            raise ValueError("Checked horizontal-axis search drifted")
        status = "verified"
    else:
        if OUTPUT.exists() or OUTPUT.is_symlink():
            raise ValueError("Horizontal-axis search exists; use --verify")
        dump_canonical_json(OUTPUT, payload)
        status = "written"
    verify_horizontal_axis_grasp_search(load_strict_json(OUTPUT))
    print(
        json.dumps(
            {
                "status": status,
