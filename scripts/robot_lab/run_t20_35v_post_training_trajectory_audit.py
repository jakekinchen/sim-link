#!/usr/bin/env python3
"""Run or verify the one T20.35v post-training trajectory audit."""
...
def main() -> int:
    ...
    if sum((args.verify, args.preflight, args.runtime_preflight)) > 1:
        raise ValueError("T20.35v choose one verification mode")
    ...
    if spec.get("required_python_major_minor") != [3, 12] or list(
        ...
        raise RuntimeError("T20.35v requires the reviewed Python 3.12 runtime")
    ...
    if (
        ...
        raise ValueError("T20.35v inherited authority identity drifted")
    if _file_tree(CHECKPOINT_ROOT) != spec["checkpoint_tree"]:
        raise ValueError("T20.35v checkpoint tree drifted")
    if hashlib.sha256(
        (REPO_ROOT / SAMPLER_SOURCE_PATH).read_bytes()
    ).hexdigest() != spec["sampler_source_sha256"]:
        raise ValueError("T20.35v sampler source drifted")
    target = sources["residual_report"]["target_action_chunk"]
    if args.preflight:
        if (REPO_ROOT / ATTEMPT_PATH).exists() or (REPO_ROOT / RESULT_PATH).exists():
            raise FileExistsError("T20.35v evaluation already exists")
        print(spec["identity_sha256"], permit["identity_sha256"])
        return 0
    if args.runtime_preflight:
        if any(
            (REPO_ROOT / path).exists()
            for path in (ATTEMPT_PATH, RESULT_PATH, RUNTIME_PREFLIGHT_PATH)
        ):
            raise FileExistsError("T20.35v runtime preflight or evaluation exists")
        runtime = _runtime_dependency_surface(spec)
        preflight = build_runtime_preflight(
            spec=spec,
            permit=permit,
            stack_identity=runtime["stack_identity"],
            dependency_versions=runtime["dependency_versions"],
        )
        dump_canonical_json(REPO_ROOT / RUNTIME_PREFLIGHT_PATH, preflight)
        verify_runtime_preflight(preflight, spec=spec, permit=permit)
        print(preflight["identity_sha256"], flush=True)
        return 0
    if args.verify:
        ...
        return 0
    if (REPO_ROOT / ATTEMPT_PATH).exists() or (REPO_ROOT / RESULT_PATH).exists():
        raise FileExistsError("T20.35v evaluation already exists; use --verify")
    runtime_preflight = load_strict_json(REPO_ROOT / RUNTIME_PREFLIGHT_PATH)
    verify_runtime_preflight(runtime_preflight, spec=spec, permit=permit)
    runtime = _runtime_dependency_surface(spec)
    if runtime["dependency_versions"] != runtime_preflight["dependency_versions"]:
        raise ValueError("T20.35v live runtime dependency versions drifted")

    attempt = sign_payload(
        {
            "schema_version": "scenesmith.t20_35v_evaluation_attempt.v1",
            "task_id": "T20.35v",
    ...
    if stack["identity_sha256"] != spec["lerobot_stack_identity_sha256"]:
        raise ValueError("T20.35v live LeRobot stack drifted")
    import torch
    ...
    if not torch.backends.mps.is_available():
        raise RuntimeError("T20.35v requires the authorized local MPS runtime")
    ...
    if sorted(checkpoint) != sorted(checkpoint_config["trainable_parameter_names"]):
        raise ValueError("T20.35v checkpoint key set drifted")
    for row in checkpoint_config["trainable_tensor_manifest"]:
        ...
        if (
            ...
            raise ValueError("T20.35v checkpoint tensor drifted")
    ...
    if hashlib.sha256(
        ...
        raise ValueError("T20.35v dataset target drifted")
    ...
    if (
        ...
        raise ValueError("T20.35v model parameter boundary drifted")
...
def _runtime_dependency_surface(spec):
    os.environ.update(
        {
            "HF_HUB_OFFLINE": "1",
            "TRANSFORMERS_OFFLINE": "1",
    ...
    stack = activate_lerobot_stack(repo_root=REPO_ROOT, stage="training")
    if stack["identity_sha256"] != spec["lerobot_stack_identity_sha256"]:
        raise ValueError("T20.35v runtime-preflight LeRobot stack drifted")
    ...
    from lerobot.datasets.dataset_metadata import LeRobotDatasetMetadata  # noqa: F401
    from lerobot.datasets.factory import resolve_delta_timestamps  # noqa: F401
    from lerobot.policies import make_policy, make_pre_post_processors  # noqa: F401
    from safetensors.torch import load_file  # noqa: F401
    from torch.utils.data import default_collate  # noqa: F401

    if not torch.backends.mps.is_available():
        raise RuntimeError("T20.35v runtime preflight requires local MPS")
    return {
        "stack_identity": stack["identity_sha256"],
        "dependency_versions": {
            name: version(name)
            for name in (
                "datasets",
    ...
    with torch.no_grad():
        for seed_index, seed in enumerate(INFERENCE_SEEDS):
            ...
            if base_sha != spec["base_noise_sha256_by_seed"][seed_index]:
                raise ValueError("T20.35v base noise failed exact reproduction")
            captured = []
            ...
            def capture(*args, **kwargs):
                ...
                if state is None or timestep is None:
                    raise ValueError("T20.35v denoise call signature drifted")
                ...
                if abs(observed_time - expected_time) > 1e-6:
                    raise ValueError("T20.35v live time grid drifted")
            ...
            if len(captured) != NUM_INFERENCE_STEPS:
                raise ValueError("T20.35v did not capture exactly ten steps")
            decoded = []
