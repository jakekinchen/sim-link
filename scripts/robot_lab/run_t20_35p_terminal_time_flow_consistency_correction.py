#!/usr/bin/env python3
"""Run or verify the one authorized T20.35p terminal flow correction."""
...
from scenesmith.robot_lab.t20_35p_simulation_training_authority import (  # noqa: E402
    require_active_authority,
)
from scenesmith.robot_lab.t20_35p_terminal_time_flow_consistency_correction import (  # noqa: E402
...
from scripts.robot_lab.run_t20_35c_expert_only_capacity_ceiling import (  # noqa: E402
...
RUN_ROOT = REPO_ROOT / "outputs/robot_lab/t20_35p_terminal_flow_run_001"
SUMMARY_PATH = RUN_ROOT / "run_summary.json"
ATTEMPT_PATH = RUN_ROOT / "attempt.json"
CHECKPOINT_ROOT = RUN_ROOT / "checkpoint"
CHECKPOINT_CONFIG_PATH = CHECKPOINT_ROOT / "terminal_flow_config.json"
CHECKPOINT_MODEL_PATH = CHECKPOINT_ROOT / "terminal_flow_model.safetensors"
ATTEMPT_SCHEMA_VERSION = "scenesmith.t20_35p_terminal_flow_attempt.v1"
...
def main() -> int:
    ...
    if args.verify and args.preflight:
        raise ValueError("T20.35p choose either --verify or --preflight")
    ...
    if spec.get("required_python_major_minor") != [3, 12] or list(
        ...
        raise RuntimeError("T20.35p requires the reviewed Python 3.12 runtime")
    ...
    if _file_tree(source_root) != spec["source_checkpoint_tree"]:
        raise ValueError("T20.35p source checkpoint tree drifted")
    if hashlib.sha256(
        (REPO_ROOT / SAMPLER_SOURCE_PATH).read_bytes()
    ).hexdigest() != spec["sampler_source_sha256"]:
        raise ValueError("T20.35p sampler source drifted")
    if args.preflight:
        if RUN_ROOT.exists():
            raise FileExistsError("T20.35p run root already exists")
    ...
    if args.verify:
        ...
        if summary["attempt_identity_sha256"] != attempt["identity_sha256"]:
            raise ValueError("T20.35p summary drifted from consumed attempt")
        if _file_tree(CHECKPOINT_ROOT) != summary["checkpoint_tree"]:
            raise ValueError("T20.35p output checkpoint tree drifted")
        ...
        with safe_open(CHECKPOINT_MODEL_PATH, framework="pt", device="cpu") as handle:
            if sorted(handle.keys()) != sorted(config["trainable_parameter_names"]):
                raise ValueError("T20.35p output checkpoint tensor names drifted")
        print(summary["identity_sha256"])
        return 0
    if RUN_ROOT.exists():
        raise FileExistsError("T20.35p immutable run root already exists; use --verify")
    ...
    if stack["identity_sha256"] != spec["lerobot_stack_identity_sha256"]:
        raise ValueError("T20.35p live LeRobot stack drifted")
    import torch
    ...
    if not torch.backends.mps.is_available():
        raise RuntimeError("T20.35p requires the authorized local MPS runtime")
    source_checkpoint_config = _verify_checkpoint_config(
    ...
    if sorted(checkpoint) != sorted(
        ...
        raise ValueError("T20.35p source checkpoint key set drifted")
    for row in source_checkpoint_config["trainable_tensor_manifest"]:
        ...
        if (
            ...
            raise ValueError("T20.35p source checkpoint tensor drifted")
    ...
    if target_lerobot.shape != (ACTION_HORIZON, ACTIVE_ACTION_DIMENSIONS):
        raise ValueError("T20.35p fixed dataset target shape drifted")
    ...
    if hashlib.sha256(
        ...
        raise ValueError("T20.35p fixed dataset target drifted")
    ...
    if hasattr(policy, "peft_config") or "Peft" in type(policy).__name__:
        raise ValueError("T20.35p unexpectedly constructed a PEFT policy")
    ...
    if (
        ...
        raise ValueError("T20.35p model parameter boundary drifted")
    ...
    if hashlib.sha256(
        ...
        raise ValueError("T20.35p live normalized target drifted")
    ...
    if len(examples) != CORRECTION_EXAMPLE_COUNT:
        raise ValueError("T20.35p live correction example count drifted")
    ...
    for update, example_index in enumerate(sample_order):
        ...
        if not torch.isfinite(loss):
            raise ValueError(f"T20.35p non-finite objective at update {update + 1}")
        loss.backward()
        if not all(
            ...
            raise ValueError(f"T20.35p non-finite gradient at update {update + 1}")
        norm = torch.nn.utils.clip_grad_norm_(trainable, GRADIENT_CLIP_NORM)
        if not torch.isfinite(norm):
            raise ValueError(f"T20.35p non-finite gradient norm at update {update + 1}")
        optimizer.step()
    ...
    checkpoint_config = sign_payload(
        {
            "schema_version": "scenesmith.t20_35p_terminal_flow_checkpoint.v1",
            "task_id": "T20.35p",
            "training_spec_identity_sha256": spec["identity_sha256"],
            "authority_decision_identity_sha256": authority_identity,
...
def _correction_loss(
    ...
    if list(losses.shape) != [1, ACTION_HORIZON, MAXIMUM_ACTION_DIMENSIONS]:
        raise ValueError("T20.35p live correction loss shape drifted")
    loss = losses[:, :, :ACTIVE_ACTION_DIMENSIONS].mean()
    if not torch.isfinite(loss):
        raise ValueError("T20.35p live correction loss is non-finite")
    return loss
...
def _standard_objectives(policy, batch, torch):
    ...
    with torch.no_grad():
        for seed in INFERENCE_SEEDS:
            ...
            loss, _ = policy(batch)
            if not torch.isfinite(loss):
                raise ValueError("T20.35p standard objective is non-finite")
    ...
    with torch.no_grad():
        for seed_index, seed in enumerate(INFERENCE_SEEDS):
            ...
            if hashlib.sha256(
                ...
                raise ValueError("T20.35p base noise failed exact reproduction")
...
def _verify_attempt(
    ...
    verify_signed_payload(payload, label="T20.35p attempt")
    if (
        ...
        raise ValueError("T20.35p attempt marker drifted")
    try:
        datetime.fromisoformat(payload["started_at"])
    except (KeyError, TypeError, ValueError) as error:
        raise ValueError("T20.35p attempt timestamp is invalid") from error


def _verify_output_checkpoint_config(payload: dict, *, summary: dict) -> dict:
    verify_signed_payload(payload, label="T20.35p output checkpoint")
    if (
        payload.get("schema_version")
        != "scenesmith.t20_35p_terminal_flow_checkpoint.v1"
        or payload.get("task_id") != "T20.35p"
