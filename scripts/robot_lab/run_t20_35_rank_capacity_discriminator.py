#!/usr/bin/env python3
"""Run or verify the one authorized T20.35 rank-16 Gate B discriminator."""
...
RUN_ROOT = REPO_ROOT / "outputs/robot_lab/t20_35_rank_capacity_run_001"
...
def main() -> int:
    ...
    if args.verify:
        ...
        if summary["attempt_identity_sha256"] != attempt["identity_sha256"]:
            raise ValueError("T20.35 summary drifted from the consumed attempt")
        if _file_tree(RUN_ROOT / "adapter") != summary["checkpoint_tree"]:
            raise ValueError("T20.35 checkpoint files drifted from run summary")
        _verify_adapter_config(RUN_ROOT / "adapter" / "adapter_config.json")
        print(summary["identity_sha256"])
        return 0
    if RUN_ROOT.exists():
        raise FileExistsError("T20.35 immutable run root already exists; use --verify")
    ...
    if not torch.backends.mps.is_available():
        raise RuntimeError("T20.35 requires the authorized local MPS runtime")
    snapshot = (
        Path.home()
    ...
    if target_lerobot.shape != (ACTION_HORIZON, 6):
        raise ValueError("T20.35 fixed dataset batch lacks the horizon-50 target")
    ...
    source_target = np.asarray(
        [
            ...
            for row in source_episode["frames"][
    ...
    if (
        ...
        raise ValueError("T20.35 dataset batch substituted the source target")

    RUN_ROOT.mkdir(parents=True, exist_ok=False)
    attempt = sign_payload(
        {
            "schema_version": "scenesmith.t20_35_rank_capacity_attempt.v1",
            "task_id": "T20.35",
    ...
    if not trainable_named:
        raise ValueError("T20.35 LoRA produced no trainable parameters")
    ...
    for update in range(OPTIMIZER_UPDATES):
        ...
        if not torch.isfinite(loss):
            raise ValueError(f"T20.35 non-finite objective at update {update + 1}")
        loss.backward()
        if not all(
            ...
            raise ValueError(f"T20.35 non-finite gradient at update {update + 1}")
        norm = torch.nn.utils.clip_grad_norm_(trainable, 1.0)
        if not torch.isfinite(norm):
            raise ValueError(
                f"T20.35 non-finite gradient norm at update {update + 1}"
            )
        optimizer.step()
    ...
    adapter_config = _verify_adapter_config(adapter_root / "adapter_config.json")
    ...
    summary = sign_payload(
        {
            ...
            "lora_rank": LORA_RANK,
            "lora_alpha": LORA_ALPHA,
            "peft_target_modules": adapter_config["target_modules"],
...
def _objective_mean(policy, batch, torch) -> float:
    ...
    with torch.no_grad():
        for seed in INFERENCE_SEEDS:
            ...
            loss, _ = policy(batch)
            if not torch.isfinite(loss):
                raise ValueError("T20.35 non-finite objective evaluation")
...
def _verify_adapter_config(path: Path) -> dict:
    payload = json.loads(path.read_text(encoding="utf-8"))
    if (
        payload.get("r") != LORA_RANK
        or payload.get("lora_alpha") != LORA_ALPHA
        or payload.get("peft_type") != "LORA"
        or payload.get("target_modules") != EXPECTED_PEFT_TARGET_MODULES
    ):
        raise ValueError("T20.35 saved adapter rank, alpha, or target modules drifted")
    return payload
...
def _verify_attempt(
    ...
    verify_signed_payload(payload, label="T20.35 rank-capacity attempt")
    if (
        payload.get("schema_version")
        != "scenesmith.t20_35_rank_capacity_attempt.v1"
        ...
        raise ValueError("T20.35 attempt marker drifted")
    try:
        datetime.fromisoformat(payload["started_at"])
    except (KeyError, TypeError, ValueError) as error:
        raise ValueError("T20.35 attempt timestamp is invalid") from error
