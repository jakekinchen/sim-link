#!/usr/bin/env python3
"""Run or verify the one T20.35l active-versus-padded noise-mask evaluation."""
...
def main() -> int:
    ...
    if spec.get("required_python_major_minor") != [3, 12] or list(
        ...
        raise RuntimeError("T20.35l requires the reviewed Python 3.12 runtime")
    ...
    if (
        ...
        raise ValueError("T20.35l inherited authority identity drifted")
    if _file_tree(REPO_ROOT / CHECKPOINT_ROOT) != spec["checkpoint_tree"]:
        raise ValueError("T20.35l checkpoint tree drifted before evaluation")
    if hashlib.sha256(
        (REPO_ROOT / SAMPLER_SOURCE_PATH).read_bytes()
    ).hexdigest() != spec["sampler_source_sha256"]:
        raise ValueError("T20.35l sampler source drifted before evaluation")
    ...
    if (REPO_ROOT / ATTEMPT_PATH).exists() or (REPO_ROOT / RESULT_PATH).exists():
        raise FileExistsError("T20.35l evaluation already exists; use --verify")
    ...
    if stack["identity_sha256"] != spec["lerobot_stack_identity_sha256"]:
        raise ValueError("T20.35l live LeRobot stack drifted")
    import torch
    ...
    if not torch.backends.mps.is_available():
        raise RuntimeError("T20.35l requires the authorized local MPS runtime")
    ...
    if sorted(checkpoint) != sorted(checkpoint_config["trainable_parameter_names"]):
        raise ValueError("T20.35l checkpoint key set drifted")
    for row in checkpoint_config["trainable_tensor_manifest"]:
        ...
        if (
            ...
            raise ValueError("T20.35l checkpoint tensor drifted or is non-finite")
    ...
    if hashlib.sha256(
        ...
        raise ValueError("T20.35l dataset target drifted")
    ...
    if hasattr(policy, "peft_config") or "Peft" in type(policy).__name__:
        raise ValueError("T20.35l unexpectedly constructed a PEFT policy")
    if (
        ...
        raise ValueError("T20.35l cadence or action width drifted")
    ...
    if (
        ...
        raise ValueError("T20.35l model parameter boundary drifted")
    ...
    evaluations = _decoded_conditions(
    ...
    result = build_result(
        ...
        mixed_condition_evaluations=evaluations,
    )
...
def _decoded_conditions(policy, preprocessor, postprocessor, raw_batch, torch, spec):
    observation = {
    ...
    policy.eval()
    with torch.no_grad():
        for condition in MIXED_NOISE_CONDITIONS:
            mask = torch.tensor(
                noise_mask_for_condition(
                    condition,
            ...
            evaluations.append(
                {"noise_condition": condition, "decoded_action_chunks": rows}
            )
    return evaluations
