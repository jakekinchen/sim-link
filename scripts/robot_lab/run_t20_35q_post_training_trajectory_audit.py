#!/usr/bin/env python3
"""Run or verify the one T20.35q post-training trajectory audit."""
...
from scenesmith.robot_lab.t20_35q_post_training_trajectory_audit import (  # noqa: E402
...
def main() -> int:
    ...
    if args.verify and args.preflight:
        raise ValueError("T20.35q choose either --verify or --preflight")
    ...
    if spec.get("required_python_major_minor") != [3, 12] or list(
        ...
        raise RuntimeError("T20.35q requires the reviewed Python 3.12 runtime")
    ...
    if args.verify:
        ...
        return 0
    if (REPO_ROOT / ATTEMPT_PATH).exists() or (REPO_ROOT / RESULT_PATH).exists():
        raise FileExistsError("T20.35q evaluation already exists; use --verify")
    ...
    if stack["identity_sha256"] != spec["lerobot_stack_identity_sha256"]:
        raise ValueError("T20.35q live LeRobot stack drifted")
    import torch
    ...
    if not torch.backends.mps.is_available():
        raise RuntimeError("T20.35q requires the authorized local MPS runtime")
    ...
    if sorted(checkpoint) != sorted(checkpoint_config["trainable_parameter_names"]):
        raise ValueError("T20.35q checkpoint key set drifted")
    for row in checkpoint_config["trainable_tensor_manifest"]:
        ...
        if (
            ...
            raise ValueError("T20.35q checkpoint tensor drifted")
    ...
    if hashlib.sha256(
        ...
        raise ValueError("T20.35q dataset target drifted")
    ...
    if (
        ...
        raise ValueError("T20.35q model parameter boundary drifted")
    ...
    with torch.no_grad():
        for seed_index, seed in enumerate(INFERENCE_SEEDS):
            ...
            if base_sha != spec["base_noise_sha256_by_seed"][seed_index]:
                raise ValueError("T20.35q base noise failed exact reproduction")
            captured = []
            ...
            def capture(*args, **kwargs):
                ...
                if state is None or timestep is None:
                    raise ValueError("T20.35q denoise call signature drifted")
                ...
                if abs(observed_time - expected_time) > 1e-6:
                    raise ValueError("T20.35q live time grid drifted")
            ...
            if len(captured) != NUM_INFERENCE_STEPS:
                raise ValueError("T20.35q did not capture exactly ten steps")
            decoded = []
