#!/usr/bin/env python3
"""Write or verify reviewer acceptance for T20.43b authority epoch 2."""
...
from scenesmith.robot_lab.t20_43b_r1_act_authority_refresh import (  # noqa: E402
...
def main() -> int:
    ...
    permit = load_strict_json(REPO_ROOT / REFRESH_PERMIT_PATH)
    if args.verify:
        payload = load_strict_json(REPO_ROOT / REFRESH_ACCEPTANCE_PATH)
        verify_refresh_acceptance(payload, permit=permit)
    else:
        if not all(
        ...
        if not reviewer.is_file() or reviewer.is_symlink():
            raise FileNotFoundError("T20.43b epoch-2 reviewer is absent or aliased")
        payload = build_refresh_acceptance(
        ...
        target = REPO_ROOT / REFRESH_ACCEPTANCE_PATH
        if os.path.lexists(target):
            raise FileExistsError("T20.43b epoch-2 acceptance already exists")
        dump_canonical_json(target, payload)
