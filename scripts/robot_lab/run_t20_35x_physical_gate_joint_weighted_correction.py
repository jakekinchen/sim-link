#!/usr/bin/env python3
"""Run or verify the one authorized T20.35x physical-gate joint-weighted correction."""
...
RUN_ROOT = REPO_ROOT / "outputs/robot_lab/t20_35x_physical_gate_joint_weighted_run_001"
SUMMARY_PATH = RUN_ROOT / "run_summary.json"
ATTEMPT_PATH = RUN_ROOT / "attempt.json"
CHECKPOINT_ROOT = RUN_ROOT / "checkpoint"
CHECKPOINT_CONFIG_PATH = CHECKPOINT_ROOT / "physical_gate_joint_weighted_config.json"
CHECKPOINT_MODEL_PATH = (
    CHECKPOINT_ROOT / "physical_gate_joint_weighted_model.safetensors"
)
ATTEMPT_SCHEMA_VERSION = "scenesmith.t20_35x_physical_gate_joint_weighted_attempt.v1"
...
def main() -> int:
    ...
    if args.verify and args.preflight:
        raise ValueError("T20.35x choose either --verify or --preflight")
    ...
    if spec.get("required_python_major_minor") != [3, 12] or list(
        ...
        raise RuntimeError("T20.35x requires the reviewed Python 3.12 runtime")
    authority = require_active_authority(repo_root=REPO_ROOT)
    authority_identity = authority["decision"]["identity_sha256"]
    runtime_preflight = load_strict_json(REPO_ROOT / RUNTIME_PREFLIGHT_PATH)
    verify_runtime_preflight(
    ...
    if _file_tree(source_root) != spec["source_checkpoint_tree"]:
        raise ValueError("T20.35x source checkpoint tree drifted")
    if (
        hashlib.sha256((REPO_ROOT / SAMPLER_SOURCE_PATH).read_bytes()).hexdigest()
    ...
    if args.verify:
        ...
        if (
            summary["runtime_preflight_identity_sha256"]
            != runtime_preflight["identity_sha256"]
            or summary["training_permit_identity_sha256"] != permit["identity_sha256"]
        ):
            raise ValueError("T20.35x summary drifted from preflight or permit")
        if summary["attempt_identity_sha256"] != attempt["identity_sha256"]:
            raise ValueError("T20.35x summary drifted from consumed attempt")
        if _file_tree(CHECKPOINT_ROOT) != summary["checkpoint_tree"]:
            raise ValueError("T20.35x output checkpoint tree drifted")
        ...
        with safe_open(CHECKPOINT_MODEL_PATH, framework="pt", device="cpu") as handle:
            if sorted(handle.keys()) != sorted(config["trainable_parameter_names"]):
                raise ValueError("T20.35x output checkpoint tensor names drifted")
        print(summary["identity_sha256"])
        return 0
    if RUN_ROOT.exists():
        raise FileExistsError("T20.35x immutable run root already exists; use --verify")

    correction_tensors = materialize_correction_tensors(
        sources["trajectory_result"],
        sources["target_result"],
    ...
    if sorted(checkpoint) != sorted(
        ...
        raise ValueError("T20.35x source checkpoint key set drifted")
    for row in source_checkpoint_config["trainable_tensor_manifest"]:
        ...
        if (
            ...
            raise ValueError("T20.35x source checkpoint tensor drifted")
    ...
    if target_lerobot.shape != (ACTION_HORIZON, ACTIVE_ACTION_DIMENSIONS):
        raise ValueError("T20.35x fixed dataset target shape drifted")
    ...
    if (
        ...
        != spec["dataset_action_chunk_sha256"]
    ):
        raise ValueError("T20.35x fixed dataset target drifted")
    ...
    if hasattr(policy, "peft_config") or "Peft" in type(policy).__name__:
        raise ValueError("T20.35x unexpectedly constructed a PEFT policy")
    ...
    if (
        ...
        raise ValueError("T20.35x model parameter boundary drifted")
    ...
    normalized_target = actions[0].detach().cpu().float().numpy().astype(float).tolist()
    if (
        hashlib.sha256(canonical_json_bytes(normalized_target)).hexdigest()
        != spec["normalized_padded_target_sha256"]
    ):
        raise ValueError("T20.35x live normalized target drifted")
    ...
    if len(examples) != CORRECTION_EXAMPLE_COUNT:
        raise ValueError("T20.35x live correction example count drifted")
    ...
    baseline_raw_by_example, baseline_joint_by_example = _correction_objectives(
        policy,
        images,
        img_masks,
        tokens,
    ...
    baseline_time_joint_by_example = [
        value * weights[index % 10]
        for index, value in enumerate(baseline_joint_by_example)
    ]
    ...
    joint_weighted_correction_losses: list[float] = []
    time_joint_weighted_correction_losses: list[float] = []
    ...
    for update, example_index in enumerate(sample_order):
        ...
        raw_loss, joint_loss = _correction_losses(
            policy,
            images,
            img_masks,
            tokens,
        ...
        if not torch.isfinite(raw_loss) or not torch.isfinite(joint_loss):
            raise ValueError(
                f"T20.35x non-finite correction objective at update {update + 1}"
            )
        time_joint_loss = joint_loss * weights[example_index % 10]
        time_joint_loss.backward()
        ...
        if not torch.isfinite(standard_loss):
            raise ValueError(
                f"T20.35x non-finite standard replay at update {update + 1}"
            )
        ...
        if not all(
            ...
            raise ValueError(f"T20.35x non-finite gradient at update {update + 1}")
        norm = torch.nn.utils.clip_grad_norm_(trainable, GRADIENT_CLIP_NORM)
        if not torch.isfinite(norm):
            raise ValueError(f"T20.35x non-finite gradient norm at update {update + 1}")
        optimizer.step()
        torch.mps.synchronize()
        raw_value = float(raw_loss.detach().cpu())
        joint_value = float(joint_loss.detach().cpu())
        time_joint_value = float(time_joint_loss.detach().cpu())
        standard_value = float(standard_loss.detach().cpu())
        raw_correction_losses.append(raw_value)
        joint_weighted_correction_losses.append(joint_value)
        time_joint_weighted_correction_losses.append(time_joint_value)
        standard_replay_losses.append(standard_value)
        losses.append(time_joint_value + standard_value)
        gradients.append(float(norm.detach().cpu()))
        if (update + 1) % 25 == 0:
            print(update + 1, losses[-1], flush=True)
    final_raw_by_example, final_joint_by_example = _correction_objectives(
        policy,
        images,
        img_masks,
        tokens,
    ...
    final_time_joint_by_example = [
        value * weights[index % 10]
        for index, value in enumerate(final_joint_by_example)
    ]
    ...
    checkpoint_config = sign_payload(
        {
            "schema_version": "scenesmith.t20_35x_physical_gate_joint_weighted_checkpoint.v1",
            "task_id": "T20.35x",
            "training_spec_identity_sha256": spec["identity_sha256"],
            "authority_decision_identity_sha256": authority_identity,
            "runtime_preflight_identity_sha256": runtime_preflight["identity_sha256"],
    ...
    summary = sign_payload(
        {
            ...
            "joint_weight_coefficient_by_dimension": spec["physical_joint_weighting"][
            ...
            "paligemma_trainable_parameter_count": 0,
            "baseline_raw_correction_objective_mean": sum(baseline_raw_by_example)
            / CORRECTION_EXAMPLE_COUNT,
            "final_raw_correction_objective_mean": sum(final_raw_by_example)
            / CORRECTION_EXAMPLE_COUNT,
            "baseline_raw_objective_by_example": baseline_raw_by_example,
            "final_raw_objective_by_example": final_raw_by_example,
            "baseline_joint_weighted_correction_objective_mean": sum(
                baseline_joint_by_example
            )
            / CORRECTION_EXAMPLE_COUNT,
            "final_joint_weighted_correction_objective_mean": sum(
                final_joint_by_example
            )
            / CORRECTION_EXAMPLE_COUNT,
            "baseline_joint_weighted_objective_by_example": baseline_joint_by_example,
            "final_joint_weighted_objective_by_example": final_joint_by_example,
            ...
            / CORRECTION_EXAMPLE_COUNT,
            "baseline_time_joint_weighted_objective_by_example": baseline_time_joint_by_example,
            "final_time_joint_weighted_objective_by_example": final_time_joint_by_example,
            ...
            "per_update_joint_weighted_correction_objective": joint_weighted_correction_losses,
            "per_update_time_joint_weighted_correction_objective": time_joint_weighted_correction_losses,
...
def _correction_losses(
    ...
    if list(losses.shape) != [1, ACTION_HORIZON, MAXIMUM_ACTION_DIMENSIONS]:
        raise ValueError("T20.35x live correction loss shape drifted")
    raw_loss = losses[:, :, :ACTIVE_ACTION_DIMENSIONS].mean()
    joint_weighted_loss = (losses * joint_coefficients).mean()
    if not torch.isfinite(raw_loss) or not torch.isfinite(joint_weighted_loss):
        raise ValueError("T20.35x live correction loss is non-finite")
    return raw_loss, joint_weighted_loss
...
def _correction_objectives(
    ...
    raw_values = []
    joint_weighted_values = []
    policy.eval()
    with torch.no_grad():
        for example in examples:
            raw_loss, joint_weighted_loss = _correction_losses(
                policy,
                images,
                img_masks,
                tokens,
            ...
            raw_values.append(float(raw_loss.detach().cpu()))
            joint_weighted_values.append(float(joint_weighted_loss.detach().cpu()))
    return raw_values, joint_weighted_values
    ...
    with torch.no_grad():
        for seed in INFERENCE_SEEDS:
            ...
            if not torch.isfinite(loss):
                raise ValueError("T20.35x standard objective is non-finite")
            values.append(float(loss.detach().cpu()))
    return values


def _decoded_chunks(
    policy, preprocessor, postprocessor, raw_batch, target, torch, spec
):
    observation = {
    ...
    with torch.no_grad():
        for seed_index, seed in enumerate(INFERENCE_SEEDS):
            ...
            if (
                ...
                != spec["evaluation"]["base_noise_sha256_by_seed"][seed_index]
            ):
                raise ValueError("T20.35x base noise failed exact reproduction")
...
def _verify_attempt(
    ...
    *,
    ...
    verify_signed_payload(payload, label="T20.35x attempt")
    if (
        ...
        or payload.get("training_permit_identity_sha256") != permit_identity
        ...
        raise ValueError("T20.35x attempt marker drifted")
    try:
        datetime.fromisoformat(payload["started_at"])
    except (KeyError, TypeError, ValueError) as error:
        raise ValueError("T20.35x attempt timestamp is invalid") from error


def _verify_output_checkpoint_config(payload: dict, *, summary: dict) -> dict:
    verify_signed_payload(payload, label="T20.35x output checkpoint")
    if (
        payload.get("schema_version")
        != "scenesmith.t20_35x_physical_gate_joint_weighted_checkpoint.v1"
        or payload.get("task_id") != "T20.35x"
        ...
        != summary["training_permit_identity_sha256"]
