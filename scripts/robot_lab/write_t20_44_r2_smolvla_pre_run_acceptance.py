#!/usr/bin/env python3
"""Write or verify signed Reviewer acceptance for the sole T20.44 attempt."""
...
from scenesmith.robot_lab.t20_44_r2_smolvla_contracts import (  # noqa: E402
    PERMIT_PATH,
...
def main() -> int:
    ...
    if args.verify:
        ...
        if not reviewer.is_file() or reviewer.is_symlink():
            raise FileNotFoundError("T20.44 reviewer decision is absent or aliased")
        ...
        if (REPO_ROOT / PRE_RUN_ACCEPTANCE_PATH).exists():
            raise FileExistsError("T20.44 pre-run acceptance already exists")
