...
1. Produce one immutable metric workcell/calibration export receipt.
2. Validate it independently in this consumer without importing source code.
3. Compile a new twin candidate with explicit coordinate transforms and
   uncertainty.
4. Keep it candidate-only until structural, camera, timing, contact, and
   held-out twin evidence compose centrally.
...
- metric calibration and hardware identity are current;
- the learned simulation policy has a selected, source-bound checkpoint;
- the central composer grants the required physical state;
- same-thread runtime verification, owner presence, finite camera/serial/robot
  permits, motion bounds, abort rules, and evidence retention all agree.
...
- Do not add another training rung after a same-shape failure unless a new
  discriminator predicts a different outcome.
- Do not change a gate after seeing a result unless the amendment was
...
The useful MVP is not “training runs.” It is the conjunction of:

1. one learned policy passing strict-v2 in MuJoCo under the fixed evaluator;
