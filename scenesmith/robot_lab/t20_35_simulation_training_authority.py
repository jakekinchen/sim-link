"""Central training-only authority for the T20.35 rank-capacity run."""
...
OWNER_GRANT_PATH = Path(
    "configurations/robot_lab/t20_35_owner_training_authorization.json"
)
REQUEST_PATH = Path(
    "configurations/robot_lab/t20_35_simulation_training_authority_request.json"
)
DECISION_PATH = Path(
    "configurations/robot_lab/t20_35_simulation_training_authority_decision.json"
)
VALID_FROM = "2026-07-15T07:27:47-05:00"
VALID_UNTIL = "2026-07-15T15:27:47-05:00"
EVALUATION_TIME = "2026-07-15T07:45:00-05:00"
AUTHORIZED_ACTIONS = (
    ...
    return sign_payload(
        {
            ...
            "authorization_id": "t20_35_gate_b_rank_capacity_grant",
            "subject_id": DEFAULT_AUTHORITY_SUBJECT_ID,
            "scope_id": DEFAULT_AUTHORITY_SCOPE_ID,
            "authorization_scope": "t20_35_simulation_only_rank_16_gate_b",
...
def verify_owner_grant(
    ...
    *,
    ...
    verify_signed_payload(payload, label="T20.35 owner training grant")
    if payload != build_owner_grant(
        training_spec=training_spec, repo_root=repo_root
    ):
        raise ValueError("T20.35 owner grant drifted from fixed simulation-only scope")
...
def build_production_authority(
    *, repo_root: Path = REPO_ROOT
    ...
    verify_signed_payload(dataset_manifest, label="T20.35 T20.17 dataset manifest")
    ...
    for prerequisite_id, evidence_refs in sorted(evidence.items()):
        ...
        claims.append(
            build_capability_claim(
                claim_id=f"t20_35_{prerequisite_id}",
                ...
                evidence_refs=[
                    build_evidence_ref(
                        artifact_kind="t20_35_rank_capacity_gate_b_evidence",
    ...
    if decision.get("authority_granted") != ["simulation_training_ready"]:
        raise ValueError("T20.35 central authority exceeded training-only scope")
...
def verify_authority(*, repo_root: Path = REPO_ROOT) -> dict[str, Any]:
    ...
    if request != expected_request:
        raise ValueError("T20.35 authority request drifted from live sources")
    verify_authority_decision(decision, request=request)
    if decision != expected_decision:
        raise ValueError("T20.35 authority decision drifted from recomposition")
...
def require_active_authority(
    *, repo_root: Path = REPO_ROOT, now: datetime | None = None
    ...
    if current < datetime.fromisoformat(VALID_FROM):
        raise ValueError("T20.35 authority is not active yet")
    if current > datetime.fromisoformat(VALID_UNTIL):
        raise ValueError("T20.35 authority has expired")
    return result
