"""Central training-only authority for the T20.35c expert-only run."""
...
OWNER_GRANT_PATH = Path(
    "configurations/robot_lab/t20_35c_owner_training_authorization.json"
)
REQUEST_PATH = Path(
    "configurations/robot_lab/t20_35c_simulation_training_authority_request.json"
)
DECISION_PATH = Path(
    "configurations/robot_lab/t20_35c_simulation_training_authority_decision.json"
)
VALID_FROM = "2026-07-15T07:27:47-05:00"
VALID_UNTIL = "2026-07-15T15:27:47-05:00"
EVALUATION_TIME = "2026-07-15T08:45:00-05:00"
AUTHORIZED_ACTIONS = (
    ...
    return sign_payload(
        {
            ...
            "authorization_id": "t20_35c_gate_b_expert_only_capacity_grant",
            "subject_id": DEFAULT_AUTHORITY_SUBJECT_ID,
            "scope_id": DEFAULT_AUTHORITY_SCOPE_ID,
            "authorization_scope": "t20_35c_simulation_only_expert_capacity_gate_b",
...
def verify_owner_grant(
    ...
    *,
    ...
    verify_signed_payload(payload, label="T20.35c owner training grant")
    if payload != build_owner_grant(
        training_spec=training_spec, repo_root=repo_root
    ):
        raise ValueError("T20.35c owner grant drifted from simulation-only scope")
...
def build_production_authority(
    *, repo_root: Path = REPO_ROOT
    ...
    verify_signed_payload(correction, label="T20.35c coverage correction")
    dataset_manifest = load_strict_json(root / DATASET_MANIFEST_PATH)
    verify_signed_payload(dataset_manifest, label="T20.35c dataset manifest")
    ...
    refs = {
        ...
        "correction": artifact_ref(
            path=CORRECTION_PATH, payload=correction, repo_root=root
        ),
    ...
    evidence = {
        ...
        "structural_contract_valid": [refs["inherited"], refs["spec"], refs["correction"]],
        "executable_stack_valid": [refs["inherited"], refs["spec"], refs["correction"]],
    ...
    for prerequisite_id, evidence_refs in sorted(evidence.items()):
        ...
        claims.append(
            build_capability_claim(
                claim_id=f"t20_35c_{prerequisite_id}",
                ...
                evidence_refs=[
                    build_evidence_ref(
                        artifact_kind="t20_35c_expert_only_gate_b_evidence",
    ...
    if decision.get("authority_granted") != ["simulation_training_ready"]:
        raise ValueError("T20.35c central authority exceeded training-only scope")
...
def verify_authority(*, repo_root: Path = REPO_ROOT) -> dict[str, Any]:
    ...
    if request != expected_request:
        raise ValueError("T20.35c authority request drifted from live sources")
    verify_authority_decision(decision, request=request)
    if decision != expected_decision:
        raise ValueError("T20.35c authority decision drifted from recomposition")
...
def require_active_authority(
    *, repo_root: Path = REPO_ROOT, now: datetime | None = None
    ...
    if current < datetime.fromisoformat(VALID_FROM):
        raise ValueError("T20.35c authority is not active yet")
    if current > datetime.fromisoformat(VALID_UNTIL):
        raise ValueError("T20.35c authority has expired")
    return result
