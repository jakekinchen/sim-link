"""Post-training trajectory comparison for T20.35q."""
...
from scenesmith.robot_lab.t20_35p_terminal_time_flow_consistency_correction import (
    RESULT_PATH as T20_35P_RESULT_PATH,
    SPEC_PATH as T20_35P_SPEC_PATH,
    verify_result as verify_t20_35p_result,
    verify_run as verify_t20_35p_run,
)


REPO_ROOT = Path(__file__).resolve().parents[2]
SPEC_PATH = Path("configurations/robot_lab/t20_35q_post_training_trajectory_spec.json")
PERMIT_PATH = Path(
    "configurations/robot_lab/t20_35q_post_training_trajectory_evaluation_permit.json"
)
RESULT_PATH = Path(
    "configurations/robot_lab/t20_35q_post_training_trajectory_result.json"
)
ATTEMPT_PATH = Path("outputs/robot_lab/t20_35q_post_training_trajectory/attempt.json")
SCHEMA_VERSION = "scenesmith.t20_35q_post_training_trajectory_spec.v1"
PERMIT_SCHEMA_VERSION = (
    "scenesmith.t20_35q_post_training_trajectory_evaluation_permit.v1"
)
RESULT_SCHEMA_VERSION = "scenesmith.t20_35q_post_training_trajectory_result.v1"
ATTEMPT_SCHEMA_VERSION = "scenesmith.t20_35q_evaluation_attempt.v1"
...
INHERITED_AUTHORITY_IDENTITY = (
    "c69090da6c813810aaee6efd7d80c20bb5ae17158947fc4543bfab291564e5d0"
)
...
def build_evaluation_spec(
    *,
    ...
    for label, payload in (
        ...
        verify_signed_payload(payload, label=f"T20.35q {label}")
    if (
        source_result.get("flow_residual_classification")
        ...
        raise ValueError("T20.35q source route drifted")
    source_rows = _seed_rows(source_result.get("trajectory_evaluations"))
    ...
    for index, (source_row, new_row) in enumerate(
        ...
        if (
            ...
            raise ValueError("T20.35q source seed order drifted")
    ...
    return sign_payload(
        {
            ...
            "task_id": "T20.35q",
            "scope": "one_inference_only_post_training_source_new_target_trajectory_comparison",
            "t20_35o_spec_identity_sha256": source_spec["identity_sha256"],
            "t20_35o_result_identity_sha256": source_result["identity_sha256"],
            "t20_35p_run_identity_sha256": training_run["identity_sha256"],
            "t20_35p_result_identity_sha256": training_result["identity_sha256"],
            ...
            "normalized_padded_target_sha256": source_result[
...
def verify_evaluation_spec(
    ...
    verify_signed_payload(payload, label="T20.35q trajectory spec")
    verify_signed_payload(expected_spec, label="T20.35q expected trajectory spec")
    if payload != expected_spec:
        raise ValueError("T20.35q trajectory spec drifted")


def build_evaluation_permit(*, spec: dict[str, Any]) -> dict[str, Any]:
    verify_signed_payload(spec, label="T20.35q permit source spec")
    if (
        ...
        raise ValueError("T20.35q permit source spec drifted")
    return sign_payload(
        {
...
def verify_evaluation_permit(
    ...
    verify_signed_payload(payload, label="T20.35q trajectory permit")
    if payload != build_evaluation_permit(spec=spec):
        raise ValueError("T20.35q trajectory permit drifted")


def verify_attempt_marker(
    payload: dict[str, Any], *, spec_identity: str, permit_identity: str
) -> None:
    verify_signed_payload(payload, label="T20.35q trajectory attempt")
    if (
        payload.get("schema_version") != ATTEMPT_SCHEMA_VERSION
        or payload.get("task_id") != "T20.35q"
        ...
        raise ValueError("T20.35q trajectory attempt drifted")
...
def build_result(
    *,
    ...
    verify_signed_payload(spec, label="T20.35q result spec")
    verify_signed_payload(source_trajectory_result, label="T20.35q source path")
    verify_signed_payload(training_result, label="T20.35q training result")
    ...
    if (
        ...
        != spec["t20_35o_result_identity_sha256"]
        or training_result["identity_sha256"]
        != spec["t20_35p_result_identity_sha256"]
        or _matrix_sha(target_chunk) != spec["dataset_action_chunk_sha256"]
    ):
        raise ValueError("T20.35q result source lineage drifted")
    target = _finite_matrix(
        source_trajectory_result.get("normalized_padded_target"),
        rows=50,
        columns=MAXIMUM_ACTION_DIMENSIONS,
        label="normalized target",
    )
    if _matrix_sha(target) != spec["normalized_padded_target_sha256"]:
        raise ValueError("T20.35q normalized target drifted")
    source_rows = _seed_rows(source_trajectory_result.get("trajectory_evaluations"))
    ...
    for seed_index, (source_row, new_row) in enumerate(
        ...
        if (
            ...
            raise ValueError("T20.35q new trajectory seed or noise drifted")
        ...
        if decoded_sha != spec["new_decoded_action_sha256_by_seed"][seed_index]:
            raise ValueError("T20.35q decoded endpoint failed exact reproduction")
        ...
        if (
            ...
            raise ValueError("T20.35q trajectory step coverage drifted")
        steps = []
        for step_index, (source_step, new_step) in enumerate(
            ...
            if (
                ...
                raise ValueError("T20.35q trajectory time grid drifted")
    ...
    if (
        ...
        route = "design_full_path_self_consistency_correction"
    ...
    return sign_payload(
        {
            ...
            "task_id": "T20.35q",
            "scope": "one_inference_only_post_training_source_new_target_trajectory_result",
            "evaluation_spec_identity_sha256": spec["identity_sha256"],
            "evaluation_permit_identity_sha256": permit["identity_sha256"],
            "evaluation_attempt_identity_sha256": attempt["identity_sha256"],
            "t20_35o_result_identity_sha256": source_trajectory_result[
                "identity_sha256"
            ],
            "t20_35p_result_identity_sha256": training_result["identity_sha256"],
...
def verify_result(
    ...
    *,
    ...
    verify_signed_payload(payload, label="T20.35q trajectory result")
...
def load_source_artifacts(*, repo_root: Path = REPO_ROOT) -> dict[str, Any]:
    ...
    training_spec = load_strict_json(root / T20_35P_SPEC_PATH)
    from scripts.robot_lab.run_t20_35p_terminal_time_flow_consistency_correction import (
        SUMMARY_PATH,
    )

    training_run = load_strict_json(SUMMARY_PATH)
    training_result = load_strict_json(root / T20_35P_RESULT_PATH)
    verify_t20_35p_run(
        training_run,
        spec=training_spec,
        authority_identity=training_result["authority_decision_identity_sha256"],
    )
    verify_t20_35p_result(
        training_result, spec=training_spec, run=training_run
    )
    if (
        source_result["identity_sha256"] != EXPECTED_T20_35O_RESULT_IDENTITY
        or training_run["identity_sha256"] != EXPECTED_T20_35P_RUN_IDENTITY
        or training_result["identity_sha256"] != EXPECTED_T20_35P_RESULT_IDENTITY
    ):
        raise ValueError("T20.35q expected source identity drifted")
    return {
        **source,
...
def _seed_rows(value: Any) -> list[dict[str, Any]]:
    if (
        ...
        raise ValueError("T20.35q seed coverage drifted")
    return value
...
def _finite_matrix(
    ...
    if not isinstance(value, list) or len(value) != rows:
        raise ValueError(f"T20.35q {label} row count drifted")
    matrix = []
    for row in value:
        if not isinstance(row, list) or len(row) != columns:
            raise ValueError(f"T20.35q {label} column count drifted")
...
def _finite(value: Any, label: str) -> float:
    if isinstance(value, bool) or not isinstance(value, (int, float)):
        raise ValueError(f"T20.35q {label} must be finite")
    number = float(value)
...
def _sha(value: Any, label: str) -> None:
    if (
        ...
        raise ValueError(f"T20.35q {label} must be lowercase SHA-256")
