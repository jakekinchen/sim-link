"""Post-settle search with compiled pad/object principal-axis alignment."""
...
SCHEMA_VERSION = "scenesmith.principal_axis_grasp_search.v1"
SOURCE = REPO_ROOT / "configurations/robot_lab/post_yaw_settle_search.json"
ALIGNMENT_MINIMUM = PRINCIPAL_AXIS_ALIGNMENT_MINIMUM


def build_principal_axis_grasp_search() -> dict[str, Any]:
    source = load_strict_json(SOURCE)
    verify_signed_payload(source, label="source post-yaw-settle search")
    candidates = [
    ...
    return sign_payload(
        {
            ...
            "evidence_mode": "deterministic_compiled_pad_object_x_axis_alignment",
            "source_post_yaw_settle_search_identity_sha256": source[
                "identity_sha256"
            ],
            "candidate_inputs_except_wrist_roll_unchanged": True,
...
def verify_principal_axis_grasp_search(payload: dict[str, Any]) -> None:
    verify_signed_payload(payload, label="principal-axis grasp search")
    candidates = payload.get("candidates", [])
    if payload.get("training_candidate_count") != len(candidates):
        raise ValueError("Principal-axis candidate count drifted")
    if (
        not payload.get("candidate_inputs_except_wrist_roll_unchanged")
        or not payload.get("contact_physics_unchanged")
    ):
        raise ValueError("Principal-axis isolation drifted")
    if payload.get("aligned_candidate_count") != sum(
        row.get("predicted_principal_axis_alignment", 0.0) >= ALIGNMENT_MINIMUM
        for row in candidates
    ):
        raise ValueError("Principal-axis alignment count drifted")
    if payload.get("geometry_eligible_count") != sum(
        row["geometry_eligible"] for row in candidates
    ):
        raise ValueError("Principal-axis eligibility drifted")
