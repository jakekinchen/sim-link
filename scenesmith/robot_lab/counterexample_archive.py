    ...
    *, receipt_refs: list[dict[str, Any]], receipts: list[dict[str, Any]]
    ...
    for index, (ref, receipt) in enumerate(zip(receipt_refs, receipts, strict=True), start=1):
        ...
        if canonical is None:
            ...
            canonical_by_fingerprint[fingerprint] = receipt["counterexample_id"]
        elif (
            receipt.get("lifecycle_state") == "active"
            or receipt.get("canonical_counterexample_id") != canonical
            or receipt.get("duplicate_of_counterexample_id") != canonical
        ):
            raise ValueError("Counterexample duplicate reference drifted")
