"""Exact-state terminal-time flow-consistency correction for T20.35p."""
...
from scenesmith.robot_lab.t20_35o_flow_trajectory_consistency_audit import (
    ...
    ATTEMPT_PATH as T20_35O_ATTEMPT_PATH,
    MAXIMUM_ACTION_DIMENSIONS,
    RESULT_PATH as T20_35O_RESULT_PATH,
    SPEC_PATH as T20_35O_SPEC_PATH,
    load_and_verify_evaluation_files as load_t20_35o_sources,
    verify_attempt_marker as verify_t20_35o_attempt,
    verify_result as verify_t20_35o_result,
)


REPO_ROOT = Path(__file__).resolve().parents[2]
SPEC_PATH = Path(
    "configurations/robot_lab/t20_35p_terminal_time_flow_consistency_training_spec.json"
)
RESULT_PATH = Path(
    "configurations/robot_lab/t20_35p_terminal_time_flow_consistency_result.json"
)
SCHEMA_VERSION = "scenesmith.t20_35p_terminal_time_flow_consistency_training_spec.v1"
RUN_SCHEMA_VERSION = "scenesmith.t20_35p_terminal_time_flow_consistency_run.v1"
RESULT_SCHEMA_VERSION = "scenesmith.t20_35p_terminal_time_flow_consistency_result.v1"
EXPECTED_T20_35C_SPEC_IDENTITY = (
    "6c10a1c7ad1f901c8afd5452669a0f0ca845d9395fa75de8e84139b706e92309"
)
EXPECTED_T20_35C_RESULT_IDENTITY = (
...
def build_correction_examples(
    ...
    verify_signed_payload(trajectory_result, label="T20.35p trajectory source")
    if (
        trajectory_result.get("identity_sha256")
        != EXPECTED_T20_35O_RESULT_IDENTITY
        or trajectory_result.get("flow_residual_classification")
        ...
        raise ValueError("T20.35p trajectory route drifted")
    target = _finite_matrix(
        trajectory_result.get("normalized_padded_target"),
        rows=50,
        columns=MAXIMUM_ACTION_DIMENSIONS,
        label="normalized padded target",
    )
    if _matrix_sha(target) != trajectory_result.get(
        "normalized_padded_target_sha256"
    ):
        raise ValueError("T20.35p normalized target hash drifted")
    rows = trajectory_result.get("trajectory_evaluations")
    if (
    ...
    for row in rows:
        ...
        for step_index in CORRECTION_STEP_INDICES:
            ...
            if _matrix_sha(state) != step.get("state_sha256"):
                raise ValueError("T20.35p correction state hash drifted")
            reference = [
                [
            ...
            if (
                ...
                raise ValueError("T20.35p exact-state reconstruction exceeded tolerance")
            examples.append(
                {
    ...
    if len(examples) != CORRECTION_EXAMPLE_COUNT:
        raise ValueError("T20.35p correction example count drifted")
    return examples
...
def materialize_correction_tensors(
    ...
    manifest = build_correction_examples(trajectory_result)
    target = trajectory_result["normalized_padded_target"]
    output = []
    for manifest_row, trajectory in zip(
        manifest,
        [
            (row, step)
            for row in trajectory_result["trajectory_evaluations"]
        ...
        if _matrix_sha(noise) != manifest_row["derived_noise_sha256"]:
            raise ValueError("T20.35p materialized correction noise drifted")
...
def build_training_spec(
    *,
    ...
    for label, payload in (
        ...
        verify_signed_payload(payload, label=f"T20.35p {label}")
    if (
        source_spec.get("identity_sha256") != EXPECTED_T20_35C_SPEC_IDENTITY
        or source_spec.get("model", {}).get("adaptation_mode") != ADAPTATION_MODE
        or source_result.get("identity_sha256") != EXPECTED_T20_35C_RESULT_IDENTITY
        or source_result.get("objective_ratio_within_threshold") is not True
        or source_result.get("gate_b_one_batch_memorization_passed") is not False
        or trajectory_result.get("identity_sha256") != EXPECTED_T20_35O_RESULT_IDENTITY
        or trajectory_result.get("evaluation_spec_identity_sha256")
        != trajectory_spec.get("identity_sha256")
    ):
        raise ValueError("T20.35p source identity or route drifted")
    examples = build_correction_examples(trajectory_result)
    ...
    if [sample_order.count(index) for index in range(CORRECTION_EXAMPLE_COUNT)] != [
        ...
        raise ValueError("T20.35p correction schedule is not balanced")
    return sign_payload(
        {
            "schema_version": SCHEMA_VERSION,
            "task_id": "T20.35p",
            "scope": "one_bounded_expert_only_exact_late_state_flow_consistency_correction",
            "t20_35c_training_spec_identity_sha256": source_spec["identity_sha256"],
            "t20_35c_result_identity_sha256": source_result["identity_sha256"],
            "t20_35o_spec_identity_sha256": trajectory_spec["identity_sha256"],
            "t20_35o_result_identity_sha256": trajectory_result["identity_sha256"],
            "source_checkpoint_identity_sha256": trajectory_spec[
                "checkpoint_identity_sha256"
            ],
            "source_checkpoint_tree": trajectory_spec["checkpoint_tree"],
            "source_checkpoint_mutated": False,
            "source_gate_baseline_objective_mean": source_result[
            ...
            "normalized_padded_target_sha256": trajectory_result[
                "normalized_padded_target_sha256"
            ],
            ...
            "campaign": {
                ...
                "sample_order": "seed_major_steps_7_8_9_repeated_30_times",
            ...
            "evaluation": {
                ...
                "source_decoded_action_sha256_by_seed": trajectory_spec[
                    "source_decoded_action_sha256_by_seed"
                ],
            },
            "gate": {
...
def verify_training_spec(
    ...
    *,
    ...
    verify_signed_payload(payload, label="T20.35p training spec")
...
def verify_run(
    ...
    if (
        ...
        or payload.get("example_use_count") != [30] * CORRECTION_EXAMPLE_COUNT
        ...
        raise ValueError("T20.35p run campaign or lineage drifted")
    for field in (
    ...
    if (
        ...
        raise ValueError("T20.35p objective or gradient trace is incomplete")
    for value in (
    ...
    if payload["baseline_correction_objective_mean"] <= 0.0:
        raise ValueError("T20.35p baseline correction objective must be positive")
    if (
        abs(
        ...
        raise ValueError("T20.35p correction objective mean drifted")
    if (
        abs(
            payload["baseline_standard_objective_mean"]
        ...
        raise ValueError("T20.35p source checkpoint standard objective drifted")
    for field in ("trainable_parameter_count", "paligemma_parameter_count"):
        if (
            ...
            raise ValueError(f"T20.35p {field} is invalid")
    ...
    if (
        ...
        raise ValueError("T20.35p decoded seed coverage drifted")
    for row in chunks:
    ...
    if not isinstance(tree, list) or len(tree) < 2:
        raise ValueError("T20.35p checkpoint tree is incomplete")
    if hashlib.sha256(canonical_json_bytes(tree)).hexdigest() != payload[
        "checkpoint_identity_sha256"
    ]:
        raise ValueError("T20.35p checkpoint identity drifted")
    required_false = (
        "checkpoint_mutated",
        "dataset_mutated",
        "statistics_changed",
    ...
    if any(payload.get(field) is not False for field in required_false):
        raise ValueError("T20.35p run authority fields drifted")


def build_result(
    *, spec: dict[str, Any], authority_identity: str, run: dict[str, Any]
) -> dict[str, Any]:
    verify_signed_payload(spec, label="T20.35p result spec")
    ...
    if gate_b_passed:
        ...
        route = "terminal_objective_pass_action_fail_route_post_training_trajectory_audit"
    elif objective_pass:
        route = "standard_objective_pass_correction_objective_fail_route_correction_attainability_audit"
    elif action_pass:
        route = "action_pass_terminal_objective_fail_route_objective_floor_audit"
    else:
        route = "terminal_correction_both_fail_route_capacity_or_interference_audit"
    return sign_payload(
        {
...
def verify_result(
    ...
    verify_signed_payload(payload, label="T20.35p result")
    ...
    if not _equal_with_float_tolerance(archived, rebuilt, absolute_tolerance=1e-15):
        raise ValueError("T20.35p result drifted")


def load_source_artifacts(*, repo_root: Path = REPO_ROOT) -> dict[str, Any]:
    root = Path(repo_root)
    source_spec = load_strict_json(root / T20_35C_SPEC_PATH)
    source_result = load_strict_json(root / T20_35C_RESULT_PATH)
    trajectory_sources = load_t20_35o_sources(repo_root=root)
    trajectory_spec = trajectory_sources["trajectory_spec"]
    trajectory_permit = trajectory_sources["trajectory_permit"]
    trajectory_attempt = load_strict_json(root / T20_35O_ATTEMPT_PATH)
    verify_t20_35o_attempt(
        trajectory_attempt,
        spec_identity=trajectory_spec["identity_sha256"],
        permit_identity=trajectory_permit["identity_sha256"],
    )
    trajectory_result = load_strict_json(root / T20_35O_RESULT_PATH)
    verify_t20_35o_result(
        trajectory_result,
        spec=trajectory_spec,
        permit=trajectory_permit,
        attempt=trajectory_attempt,
        source_result=trajectory_sources["source_result"],
...
def _finite_matrix(
    ...
    if not isinstance(value, list) or len(value) != rows:
        raise ValueError(f"T20.35p {label} row count drifted")
    matrix = []
    for row in value:
        if not isinstance(row, list) or len(row) != columns:
            raise ValueError(f"T20.35p {label} column count drifted")
...
def _finite(value: Any, label: str) -> float:
    if isinstance(value, bool) or not isinstance(value, (int, float)):
        raise ValueError(f"T20.35p {label} must be finite")
    number = float(value)
...
def _sha(value: Any, label: str) -> None:
    if (
        ...
        raise ValueError(f"T20.35p {label} must be lowercase SHA-256")
