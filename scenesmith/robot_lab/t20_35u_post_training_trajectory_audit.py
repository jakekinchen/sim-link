"""Post-training trajectory comparison for T20.35u."""
...
SPEC_PATH = Path("configurations/robot_lab/t20_35u_post_training_trajectory_spec.json")
PERMIT_PATH = Path(
    "configurations/robot_lab/t20_35u_post_training_trajectory_evaluation_permit.json"
)
RESULT_PATH = Path(
    "configurations/robot_lab/t20_35u_post_training_trajectory_result.json"
)
ATTEMPT_PATH = Path("outputs/robot_lab/t20_35u_post_training_trajectory/attempt.json")
SCHEMA_VERSION = "scenesmith.t20_35u_post_training_trajectory_spec.v1"
PERMIT_SCHEMA_VERSION = (
    "scenesmith.t20_35u_post_training_trajectory_evaluation_permit.v1"
)
RESULT_SCHEMA_VERSION = "scenesmith.t20_35u_post_training_trajectory_result.v1"
ATTEMPT_SCHEMA_VERSION = "scenesmith.t20_35u_evaluation_attempt.v1"
...
def build_evaluation_spec(
    *,
    ...
    for label, payload in (
        ...
        verify_signed_payload(payload, label=f"T20.35u {label}")
    if (
        ...
        raise ValueError("T20.35u source route drifted")
    ...
    for index, (source_row, new_row) in enumerate(
        ...
        if (
            ...
            raise ValueError("T20.35u source seed order drifted")
...
def verify_evaluation_spec(
    ...
    verify_signed_payload(payload, label="T20.35u trajectory spec")
    verify_signed_payload(expected_spec, label="T20.35u expected trajectory spec")
    if payload != expected_spec:
        raise ValueError("T20.35u trajectory spec drifted")


def build_evaluation_permit(*, spec: dict[str, Any]) -> dict[str, Any]:
    verify_signed_payload(spec, label="T20.35u permit source spec")
    if (
        ...
        raise ValueError("T20.35u permit source spec drifted")
    return sign_payload(
        {
...
def verify_evaluation_permit(
    ...
    verify_signed_payload(payload, label="T20.35u trajectory permit")
    if payload != build_evaluation_permit(spec=spec):
        raise ValueError("T20.35u trajectory permit drifted")


def verify_attempt_marker(
    payload: dict[str, Any], *, spec_identity: str, permit_identity: str
) -> None:
    verify_signed_payload(payload, label="T20.35u trajectory attempt")
    if (
        payload.get("schema_version") != ATTEMPT_SCHEMA_VERSION
        or payload.get("task_id") != "T20.35u"
        ...
        raise ValueError("T20.35u trajectory attempt drifted")
...
def build_result(
    *,
    ...
    verify_signed_payload(spec, label="T20.35u result spec")
    verify_signed_payload(source_trajectory_result, label="T20.35u source path")
    verify_signed_payload(training_result, label="T20.35u training result")
    ...
    if (
        ...
        raise ValueError("T20.35u result source lineage drifted")
    ...
    if _matrix_sha(target) != spec["normalized_padded_target_sha256"]:
        raise ValueError("T20.35u normalized target drifted")
    ...
    for seed_index, (source_row, new_row) in enumerate(
        ...
        if (
            ...
            raise ValueError("T20.35u new trajectory seed or noise drifted")
        ...
        if decoded_sha != spec["new_decoded_action_sha256_by_seed"][seed_index]:
            raise ValueError("T20.35u decoded endpoint failed exact reproduction")
        ...
        if (
            ...
            raise ValueError("T20.35u trajectory step coverage drifted")
        steps = []
        for step_index, (source_step, new_step) in enumerate(
            ...
            if (
                ...
                raise ValueError("T20.35u trajectory time grid drifted")
...
def verify_result(
    ...
    *,
    ...
    verify_signed_payload(payload, label="T20.35u trajectory result")
    ...
    if not _equal_with_float_tolerance(archived, rebuilt, absolute_tolerance=1e-15):
        raise ValueError("T20.35u trajectory result drifted")
...
def load_source_artifacts(*, repo_root: Path = REPO_ROOT) -> dict[str, Any]:
    ...
    if (
        ...
        raise ValueError("T20.35u expected source identity drifted")
    return {
        **source,
...
def _seed_rows(value: Any) -> list[dict[str, Any]]:
    if (
        ...
        raise ValueError("T20.35u seed coverage drifted")
    return value
...
def _finite_matrix(
    ...
    if not isinstance(value, list) or len(value) != rows:
        raise ValueError(f"T20.35u {label} row count drifted")
    matrix = []
    for row in value:
        if not isinstance(row, list) or len(row) != columns:
            raise ValueError(f"T20.35u {label} column count drifted")
...
def _finite(value: Any, label: str) -> float:
    if isinstance(value, bool) or not isinstance(value, (int, float)):
        raise ValueError(f"T20.35u {label} must be finite")
    number = float(value)
...
def _sha(value: Any, label: str) -> None:
    if (
        ...
        raise ValueError(f"T20.35u {label} must be lowercase SHA-256")
