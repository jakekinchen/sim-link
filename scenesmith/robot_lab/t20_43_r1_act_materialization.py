"""Model-free Gate A collection and exclusive T20.43 authority materialization."""
...
IMPLEMENTATION_SCOPED_PATHS = (
    Path("scenesmith/robot_lab/t20_43_r1_act_contracts.py"),
    Path("scenesmith/robot_lab/t20_43_r1_act_materialization.py"),
    Path("scenesmith/robot_lab/t20_43_r1_act_runner.py"),
    Path("scripts/robot_lab/write_t20_43_r1_act_spec.py"),
    Path("scripts/robot_lab/materialize_t20_43_r1_act_authority.py"),
    Path("scripts/robot_lab/write_t20_43_r1_act_pre_run_acceptance.py"),
    Path("scripts/robot_lab/run_t20_43_r1_act.py"),
    Path("scripts/robot_lab/render_rollout_mirror.py"),
    Path("tests/unit/test_t20_43_r1_act_contracts.py"),
    Path("tests/unit/test_t20_43_r1_act_runner.py"),
)
...
def collect_gate_a_evidence(
    *, spec: dict[str, Any], sources: dict[str, Any], repo_root: Path = REPO_ROOT
    ...
    if len(dataset.meta.episodes) != 129 or dataset.meta.total_frames != 31366:
        raise ValueError("T20.43 Gate A package count drifted")
    config = ACTConfig(
        input_features={
    ...
    for sample_index in SAMPLE_INDICES:
        ...
        if list(item["action"].shape) != [50, 6]:
            raise ValueError("T20.43 Gate A action horizon drifted")
        observed_shapes = {
        ...
        if observed_shapes != image_shapes or list(item["observation.state"].shape) != [
            ...
            raise ValueError("T20.43 Gate A feature shape drifted")
...
def collect_live_runtime_snapshot(
    *, required_source_commit: str, repo_root: Path = REPO_ROOT
    ...
    if branch != BRANCH:
        raise ValueError("T20.43 branch drifted")
    ...
    if diff.returncode != 0:
        raise ValueError("T20.43 reviewed implementation changed after review")
    if _sha_file(RESNET18_CACHE_PATH) != RESNET18_CACHE_SHA256:
        raise ValueError("T20.43 cached backbone drifted")
    all_paths = (*AUTHORITY_PATHS, *OUTPUT_PATHS)
    if any(os.path.lexists(root / path) for path in all_paths):
        raise FileExistsError("T20.43 authority/output artifact already exists")
    output_state = {
        path.as_posix(): {
...
def verify_materialization_bundle(
    ...
    *,
    ...
    if list(bundle) != [path.as_posix() for path in AUTHORITY_PATHS]:
        raise ValueError("T20.43 authority bundle path/order drifted")
    ...
    if request != expected_request or decision != expected_decision:
        raise ValueError("T20.43 central authority drifted")
    verify_permit(
        permit,
        spec=spec,
        gate_a=gate_a,
...
def materialize_live_authority(
    *,
    ...
    runtime = collect_live_runtime_snapshot(
        required_source_commit=required_source_commit, repo_root=root
    )
...
def _write_bundle_exclusively(
    ...
    for relative in AUTHORITY_PATHS:
        ...
        if os.path.lexists(target) or _path_or_parent_is_symlink(root, relative):
            raise FileExistsError(f"T20.43 authority target is unsafe: {relative}")
    for relative in AUTHORITY_PATHS:
        target = root / relative
...
def _require_ancestor(root: Path, ancestor: str, descendant: str) -> None:
    ...
    if result.returncode != 0:
        raise ValueError(f"T20.43 ancestry check failed: {ancestor} -> {descendant}")
