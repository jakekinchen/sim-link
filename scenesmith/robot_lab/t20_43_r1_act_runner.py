"""One-use runner and full evidence verifier for T20.43/R1 ACT."""
...
from scenesmith.robot_lab.t20_43_r1_act_materialization import (
...
TRACE_SCHEMA_VERSION = "scenesmith.t20_43_r1_act_closed_loop_trace.v1"
RUN_SCHEMA_VERSION = "scenesmith.t20_43_r1_act_run.v1"
RESULT_SCHEMA_VERSION = "scenesmith.t20_43_r1_act_result.v1"
RETENTION_SCHEMA_VERSION = "scenesmith.t20_43_r1_act_retention_receipt.v1"
SCORECARD_SCHEMA_VERSION = "scenesmith.t20_43_r1_act_scorecard.v1"
...
class ACTRolloutAdapter:
    ...
    def __init__(
        ...
        *,
        ...
        if n_action_steps not in EVALUATION_ACTION_STEPS:
            raise ValueError("T20.43 unsupported action-consumption variant")
        self.policy = policy
    ...
    def _decode(self, images: dict[str, np.ndarray], state: np.ndarray) -> None:
        ...
        if canonical_np.shape != (CHUNK_SIZE, 6) or not np.isfinite(canonical_np).all():
            raise ValueError("T20.43 ACT emitted an invalid action chunk")
        physical = np.asarray(
        ...
        if executed_length <= 0:
            raise ValueError("T20.43 decode occurred beyond the rollout")
...
def build_comparison_rows(
    *,
    ...
    if (
        ...
        raise ValueError("T20.43 complete trace inputs drifted")
    rows = []
    for index, (source, candidate) in enumerate(
        ...
        if source.get("source_phase") != candidate.get("phase"):
            raise ValueError("T20.43 source/candidate phase drifted")
        rows.append(
            {
                "frame_index": index,
                ...
                "candidate_strict_contact": bool(candidate["t20_43_strict_contact"]),
            }
        )
...
def build_trace(
    *,
    ...
    if variant_id != expected_variant:
        raise ValueError("T20.43 trace variant drifted")
    _verify_comparison_rows(rows, n_action_steps=n_action_steps)
    _verify_decode_rows(decode_rows, n_action_steps=n_action_steps)
    verify_signed_payload(closed_loop, label="T20.43 embedded rollout")
    ...
    if (
        ...
        raise ValueError("T20.43 rollout linkage drifted")
...
def verify_trace(payload: dict[str, Any]) -> None:
    verify_signed_payload(payload, label="T20.43 trace")
    if (
        payload.get("schema_version") != TRACE_SCHEMA_VERSION
        or payload.get("task_id") != "T20.43"
        ...
        raise ValueError("T20.43 trace identity or authority drifted")
    ...
    if (
        ...
        raise ValueError("T20.43 source episode reference is unsafe")
    ...
    if payload.get("adapter_id") != _variant_id(n_action_steps):
        raise ValueError("T20.43 trace variant identity drifted")
    ...
    if (
        ...
        raise ValueError("T20.43 trace derived evidence drifted")
    rollout = payload.get("closed_loop")
    verify_signed_payload(rollout, label="T20.43 embedded rollout")
    if (
        rollout.get("seed") != 0
        ...
        raise ValueError("T20.43 embedded rollout linkage drifted")
    if payload.get("t20_38_runtime_receipt") != _t20_38_runtime_receipt(rollout):
        raise ValueError("T20.43 T20.38 runtime margins drifted")
    if payload.get("model_inference_executed") is not True or any(
        ...
        raise ValueError("T20.43 trace terminal authority drifted")
...
def build_run_summary(
    *,
    ...
    if optimizer_update_count != MAXIMUM_OPTIMIZER_UPDATES:
        raise ValueError("T20.43 run did not complete the fixed update budget")
...
def verify_run_summary(payload: dict[str, Any]) -> None:
    verify_signed_payload(payload, label="T20.43 run summary")
    if payload.get("schema_version") != RUN_SCHEMA_VERSION:
        raise ValueError("T20.43 run schema drifted")
    ...
    if payload != expected:
        raise ValueError("T20.43 run summary drifted")
...
def verify_result(payload: dict[str, Any], *, run: dict[str, Any]) -> None:
    verify_signed_payload(payload, label="T20.43 result")
    if payload != build_result(run=run):
        raise ValueError("T20.43 result drifted")
...
def verify_all_outputs(*, repo_root: Path = REPO_ROOT) -> dict[str, Any]:
    ...
    for checkpoint in run["checkpoints"]:
        if _file_tree(root / checkpoint["path"]) != checkpoint["tree"]:
            raise ValueError("T20.43 checkpoint tree drifted")
    for evaluation in run["evaluations"]:
        for variant in evaluation["variants"]:
            ...
            if trace["identity_sha256"] != variant["trace_identity_sha256"]:
                raise ValueError("T20.43 trace identity drifted")
            video = root / variant["video_path"]
            manifest = load_strict_json(root / variant["video_manifest_path"])
            verify_signed_payload(manifest, label="T20.43 mirror manifest")
            if (
                not video.is_file()
                or video.is_symlink()
                ...
                raise ValueError("T20.43 mirror evidence drifted")
...
def run_authorized_attempt(
    *, started_at: str, repo_root: Path = REPO_ROOT
    ...
    if any(
        ...
        raise FileExistsError("T20.43 immutable attempt/output already exists")
    ...
    if not torch.backends.mps.is_available():
        raise RuntimeError("T20.43 requires MPS")
    ...
    def checkpoint_and_evaluate(update: int) -> None:
        ...
        for n_action_steps in EVALUATION_ACTION_STEPS:
            ...
            rollout = run_policy_grasp_closed_loop(
                ...
                seed=0,
                schema_version="scenesmith.t20_43_r1_act_rollout.v1",
                task_id="T20.43",
            ...
            for row in observed:
                row["t20_43_strict_contact"] = has_valid_antipodal_contact(
                    row, requirement
                )
    ...
    for update in range(1, MAXIMUM_OPTIMIZER_UPDATES + 1):
        ...
        if not torch.isfinite(loss):
            raise ValueError(f"T20.43 non-finite loss at update {update}")
        loss.backward()
        if not all(
            ...
            raise ValueError(f"T20.43 non-finite gradient at update {update}")
        norm = torch.nn.utils.clip_grad_norm_(policy.parameters(), 10.0)
        if not torch.isfinite(norm):
            raise ValueError(f"T20.43 non-finite gradient norm at update {update}")
        optimizer.step()
...
def _verify_checkpoint_rows(rows: Any) -> None:
    if not isinstance(rows, list) or [
        ...
        raise ValueError("T20.43 checkpoint schedule drifted")
    for row in rows:
        _sha(row.get("identity_sha256"))
        tree = row.get("tree")
        if not isinstance(tree, list) or not tree:
            raise ValueError("T20.43 checkpoint tree is empty")
        if (
            hashlib.sha256(canonical_json_bytes(tree)).hexdigest()
            != row["identity_sha256"]
        ):
            raise ValueError("T20.43 checkpoint identity drifted")
...
def _verify_evaluation_rows(rows: Any) -> None:
    if not isinstance(rows, list) or [
        ...
        raise ValueError("T20.43 evaluation schedule drifted")
    for row in rows:
        variants = row.get("variants")
        if not isinstance(variants, list) or [
            v.get("variant_id") for v in variants
        ] != list(VARIANT_ORDER):
            raise ValueError("T20.43 evaluation variant order drifted")
        for variant, action_steps in zip(
            ...
            if (
                ...
                raise ValueError("T20.43 evaluation variant semantics drifted")
            for key in (
...
def _verify_decode_rows(rows: Any, *, n_action_steps: int) -> None:
    ...
    if (
        ...
        raise ValueError("T20.43 decode starts or executed lengths drifted")
    for row, start, length in zip(rows, expected_starts, expected_lengths, strict=True):
        if (
            ...
            raise ValueError("T20.43 decode evidence drifted")
...
def _verify_comparison_rows(rows: Any, *, n_action_steps: int) -> None:
    if not isinstance(rows, list) or len(rows) != ROLLOUT_FRAMES:
        raise ValueError("T20.43 trace must contain 244 frames")
    ...
    for index, (row, phase) in enumerate(zip(rows, phases, strict=True)):
        if (
            ...
            raise ValueError("T20.43 trace frame/chunk order drifted")
        for field in (
        ...
        if not isinstance(row.get("candidate_strict_contact"), bool):
            raise ValueError("T20.43 strict contact trace drifted")
...
def _t20_38_runtime_receipt(closed_loop: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(margins, dict) or not margins:
        raise ValueError("T20.43 rollout lacks strict-v2 margins")
    if not all(isinstance(row.get("passed"), bool) for row in margins.values()):
        raise ValueError("T20.43 strict-v2 margin rows drifted")
    ...
    if strict_success is not all(row["passed"] for row in margins.values()):
        raise ValueError("T20.43 strict-v2 conjunction drifted")
    return sign_payload(
        {
            "schema_version": "scenesmith.t20_43_t20_38_runtime_margins.v1",
...
def _source_episode_zero(root: Path) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    for path in sorted(default_store_root(repo_root=root).glob("*.json")):
        ...
        if payload.get("episode_spec", {}).get("seed") == 0:
            ...
            if len(frames) != ROLLOUT_FRAMES:
                raise ValueError("T20.43 source episode zero is incomplete")
            return (
                {
    ...
    raise FileNotFoundError("T20.43 source episode zero is absent")
...
def _require_active_time(owner: dict[str, Any], started_at: str) -> None:
    ...
    if current.tzinfo is None or current.utcoffset() is None:
        raise ValueError("T20.43 start time lacks UTC offset")
    if not (
        datetime.fromisoformat(owner["valid_from"])
        <= current
        <= datetime.fromisoformat(owner["valid_until"])
    ):
        raise ValueError("T20.43 owner authority is inactive")
...
def _require_remote_preservation(
    ...
    if _git(repo_root, "branch", "--show-current") != BRANCH:
        raise ValueError("T20.43 run is on the wrong branch")
    head = _git(repo_root, "rev-parse", "HEAD")
    upstream = _git(repo_root, "rev-parse", "@{u}")
    if head != upstream:
        raise ValueError("T20.43 HEAD is not origin-confirmed")
    for ancestor in (
    ...
    if dirty.strip():
        raise ValueError("T20.43 scoped pre-run boundary is dirty")
    ...
    if (
        ...
        raise ValueError("T20.43 reviewer decision bytes drifted")


def _file_tree(root: Path) -> list[dict[str, Any]]:
    if not root.is_dir() or root.is_symlink():
        raise ValueError(f"T20.43 output tree is absent or aliased: {root}")
    rows = []
    for path in sorted(root.rglob("*")):
        if path.is_symlink():
            raise ValueError(f"T20.43 output tree contains a symlink: {path}")
        if path.is_file():
            rows.append(
                {
                    "path": path.relative_to(root).as_posix(),
    ...
    if not rows:
        raise ValueError(f"T20.43 output tree is empty: {root}")
    return rows
...
def _scheduled_update(value: Any) -> int:
    if (
        ...
        raise ValueError("T20.43 update is outside the fixed schedule")
    return value
...
def _finite_series(values: Any, *, expected: int, label: str) -> None:
    if (
        ...
        raise ValueError(f"T20.43 {label} series drifted")


def _vector(value: Any) -> list[float]:
    array = np.asarray(value, dtype=np.float64)
    if array.shape != (6,) or not np.isfinite(array).all():
        raise ValueError("T20.43 expected a finite six-joint vector")
...
def _position(value: Any) -> list[float]:
    ...
    if array.shape != (3,) or not np.isfinite(array).all():
        raise ValueError("T20.43 expected a finite xyz position")
    return array.astype(float).tolist()
...
def _sha(value: Any) -> str:
    if not isinstance(value, str) or len(value) != 64:
        raise ValueError("T20.43 expected a SHA-256 digest")
    try:
        int(value, 16)
    except ValueError as error:
        raise ValueError("T20.43 expected a SHA-256 digest") from error
    return value
