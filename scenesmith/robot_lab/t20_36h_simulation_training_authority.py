"""Central simulation-only authority for the exact T20.36h SmolVLA attempt."""
...
EVALUATION_TIME = "2026-07-15T20:00:00-05:00"
AUTHORIZED_ACTIONS = (
    ...
    return sign_payload(
        {
            ...
            "authorization_id": "t20_36h_exact_smolvla_gate_b_attempt_grant",
            "subject_id": DEFAULT_AUTHORITY_SUBJECT_ID,
            "scope_id": DEFAULT_AUTHORITY_SCOPE_ID,
            "authorization_scope": (
                "one_exact_local_smolvla_one_batch_simulation_only_gate_b_attempt"
            ),
            ...
            "authorization_source": (
                "owner_requested_autonomous_goal_loop_and_pre_registered_"
                "post_x_smolvla_branch_2026_07_15"
            ),
        }
    )
...
def verify_owner_grant(
    ...
    *,
    ...
    verify_signed_payload(payload, label="T20.36h owner training grant")
    if payload != build_owner_grant(
        training_spec=training_spec, repo_root=repo_root
    ):
        raise ValueError("T20.36h owner grant drifted")
...
def build_production_authority(
    *, repo_root: Path = REPO_ROOT
    ...
    verify_signed_payload(local_preflight, label="T20.36h local policy preflight")
    verify_signed_payload(dataset, label="T20.36h canonical dataset manifest")
    ...
    for prerequisite_id, evidence_refs in sorted(evidence.items()):
        ...
        claims.append(
            build_capability_claim(
                claim_id=f"t20_36h_{prerequisite_id}",
                ...
                evidence_refs=[
                    build_evidence_ref(
                        artifact_kind="t20_36h_exact_smolvla_evidence",
    ...
    if decision.get("authority_granted") != ["simulation_training_ready"]:
        raise ValueError("T20.36h central authority exceeded training-only scope")
...
def verify_authority(*, repo_root: Path = REPO_ROOT) -> dict[str, Any]:
    ...
    if request != expected_request:
        raise ValueError("T20.36h authority request drifted")
    verify_authority_decision(decision, request=request)
    if decision != expected_decision:
        raise ValueError("T20.36h authority decision drifted")
...
def require_active_authority(
    *, repo_root: Path = REPO_ROOT, now: datetime | None = None
    ...
    if current.tzinfo is None or current.utcoffset() is None:
        raise ValueError("T20.36h authority time must carry a UTC offset")
    if current < datetime.fromisoformat(VALID_FROM):
        raise ValueError("T20.36h authority is not active yet")
    if current > datetime.fromisoformat(VALID_UNTIL):
        raise ValueError("T20.36h authority has expired")
    return result
