"""Fail-closed contracts for the T20.43/R1 ACT standard rung.
...
SOURCE_RESULT_COMMIT = "f46c8aae0a0dc291ebc80ec1094033311fa0f717"
...
SPEC_PATH = Path("configurations/robot_lab/t20_43_r1_act_standard_spec.json")
GATE_A_PATH = Path("configurations/robot_lab/t20_43_r1_act_gate_a.json")
OWNER_GRANT_PATH = Path(
    "configurations/robot_lab/t20_43_r1_act_owner_authorization.json"
)
REQUEST_PATH = Path("configurations/robot_lab/t20_43_r1_act_authority_request.json")
DECISION_PATH = Path("configurations/robot_lab/t20_43_r1_act_authority_decision.json")
RUNTIME_PREFLIGHT_PATH = Path(
    "configurations/robot_lab/t20_43_r1_act_runtime_preflight.json"
)
PERMIT_PATH = Path("configurations/robot_lab/t20_43_r1_act_permit.json")
PRE_RUN_ACCEPTANCE_PATH = Path(
    "configurations/robot_lab/t20_43_r1_act_pre_run_acceptance.json"
)
ATTEMPT_PATH = Path("configurations/robot_lab/t20_43_r1_act_attempt.json")
RESULT_PATH = Path("configurations/robot_lab/t20_43_r1_act_result.json")
RETENTION_PATH = Path("configurations/robot_lab/t20_43_r1_act_retention_receipt.json")
SCORECARD_PATH = Path("configurations/robot_lab/t20_43_r1_act_scorecard.json")
RUN_ROOT = Path("outputs/robot_lab/t20_43_r1_act_run_001")
...
SPEC_SCHEMA_VERSION = "scenesmith.t20_43_r1_act_standard_spec.v1"
GATE_A_SCHEMA_VERSION = "scenesmith.t20_43_r1_act_gate_a.v1"
OWNER_SCHEMA_VERSION = "scenesmith.t20_43_r1_act_owner_authorization.v1"
RUNTIME_SCHEMA_VERSION = "scenesmith.t20_43_r1_act_runtime_preflight.v1"
PERMIT_SCHEMA_VERSION = "scenesmith.t20_43_r1_act_permit.v1"
ATTEMPT_SCHEMA_VERSION = "scenesmith.t20_43_r1_act_attempt.v1"
ACCEPTANCE_SCHEMA_VERSION = "scenesmith.t20_43_r1_act_pre_run_acceptance.v1"
...
def load_verified_sources(*, repo_root: Path = REPO_ROOT) -> dict[str, Any]:
    ...
    for name, expected in EXPECTED_SOURCE_IDENTITIES.items():
        verify_signed_payload(sources[name], label=f"T20.43 {name}")
        if sources[name].get("identity_sha256") != expected:
            raise ValueError(f"T20.43 reviewed source drifted: {name}")
    ...
    if (
        ...
        raise ValueError("T20.43 R0 training/held-out boundary drifted")
    ...
    if inherited_decision.get("authority_granted") != ["simulation_training_ready"]:
        raise ValueError("T20.43 inherited authority exceeded simulation scope")
...
def build_spec(*, sources: dict[str, Any]) -> dict[str, Any]:
    ...
    payload = {
        ...
        "task_id": TASK_ID,
        "scope": "one_full_act_standard_recipe_r1_with_rollout_primary_evaluation",
...
def verify_spec(payload: dict[str, Any], *, sources: dict[str, Any]) -> None:
    verify_signed_payload(payload, label="T20.43 standard spec")
    expected = build_spec(sources=sources)
    if payload != expected:
        raise ValueError("T20.43 standard spec drifted")
...
def verify_gate_a(payload: dict[str, Any], *, spec: dict[str, Any]) -> None:
    verify_signed_payload(payload, label="T20.43 Gate A")
    if payload != build_gate_a(spec=spec, evidence=payload.get("evidence")):
        raise ValueError("T20.43 Gate A drifted")
    ...
    return sign_payload(
        {
            ...
            "task_id": TASK_ID,
            "authorization_id": "t20_43_one_full_act_standard_r1_attempt",
...
def verify_owner_grant(payload: dict[str, Any], *, spec: dict[str, Any]) -> None:
    verify_signed_payload(payload, label="T20.43 owner grant")
    ...
    if payload != expected:
        raise ValueError("T20.43 owner grant drifted")
...
def build_central_authority(
    *,
    ...
    for prerequisite_id, evidence_refs in sorted(evidence.items()):
        ...
        claims.append(
            build_capability_claim(
                claim_id=f"t20_43_{prerequisite_id}",
                ...
                evidence_refs=[
                    build_evidence_ref(
                        artifact_kind="t20_43_r1_act_evidence",
    ...
    if decision.get("authority_granted") != ["simulation_training_ready"]:
        raise ValueError("T20.43 central authority exceeded simulation training")
...
def verify_runtime_preflight(
    ...
    *,
    ...
    verify_signed_payload(payload, label="T20.43 runtime preflight")
    runtime = {
        key: value
    ...
    if payload != expected:
        raise ValueError("T20.43 runtime preflight drifted")
...
def verify_permit(
    ...
    *,
    ...
    verify_signed_payload(payload, label="T20.43 permit")
    ...
    if payload != expected:
        raise ValueError("T20.43 permit drifted")
...
def build_attempt_marker(
    *, permit: dict[str, Any], source_commit: str, started_at: str
    ...
    if permit.get("authorized_attempt_count") != 1:
        raise ValueError("T20.43 permit attempt count drifted")
    return sign_payload(
        {
...
def verify_attempt_marker(payload: dict[str, Any], *, permit: dict[str, Any]) -> None:
    verify_signed_payload(payload, label="T20.43 attempt marker")
    ...
    if payload != expected:
        raise ValueError("T20.43 attempt marker drifted")
    ...
    return sign_payload(
        {
            ...
            "task_id": TASK_ID,
            "decision": "ACCEPT_ONE_T20_43_R1_ACT_STANDARD_ATTEMPT",
...
def verify_pre_run_acceptance(
    ...
    verify_signed_payload(payload, label="T20.43 pre-run acceptance")
    ...
    if payload != expected:
        raise ValueError("T20.43 pre-run acceptance drifted")
...
def _verify_sources(sources: dict[str, Any]) -> None:
    if not isinstance(sources, dict) or set(EXPECTED_SOURCE_IDENTITIES) - set(sources):
        raise ValueError("T20.43 source bundle is incomplete")
    for name, expected in EXPECTED_SOURCE_IDENTITIES.items():
        if sources[name].get("identity_sha256") != expected:
            raise ValueError(f"T20.43 source identity drifted: {name}")
    if not isinstance(sources.get("source_refs"), dict) or not isinstance(
        sources.get("act_source_files"), list
    ):
        raise ValueError("T20.43 source refs are incomplete")


def _verify_gate_a_evidence(evidence: Any) -> None:
    if not isinstance(evidence, dict):
        raise ValueError("T20.43 Gate A evidence must be an object")
    exact = {
    ...
    for key, expected in exact.items():
        if evidence.get(key) != expected:
            raise ValueError(f"T20.43 Gate A evidence drifted: {key}")
    for key in (
    ...
    for key in (
        ...
        if value > tolerance:
            raise ValueError(f"T20.43 Gate A tolerance failed: {key}")


def _verify_runtime_snapshot(snapshot: Any, *, owner_grant: dict[str, Any]) -> None:
    if not isinstance(snapshot, dict):
        raise ValueError("T20.43 runtime snapshot must be an object")
    required = {
        "source_commit",
        "branch",
    ...
    if set(snapshot) != required:
        raise ValueError("T20.43 runtime snapshot field set drifted")
    if (
        ...
        raise ValueError("T20.43 runtime facts failed closed")
    dependencies = snapshot["dependencies"]
    if dependencies != EXPECTED_DEPENDENCIES:
        raise ValueError("T20.43 dependency versions drifted")
    ...
    if set(output_state) != expected_paths or any(
        ...
        raise ValueError("T20.43 output paths are not absent and unaliased")
...
def _authority_window(valid_from: str, valid_until: str) -> tuple[datetime, datetime]:
    ...
    if seconds <= 0 or seconds > MAXIMUM_AUTHORITY_SECONDS:
        raise ValueError("T20.43 authority window is invalid")
    return start, stop


def _aware_time(value: Any, *, label: str) -> datetime:
    if not isinstance(value, str):
        raise ValueError(f"T20.43 {label} must be ISO text")
    parsed = datetime.fromisoformat(value)
    if parsed.tzinfo is None or parsed.utcoffset() is None:
        raise ValueError(f"T20.43 {label} must carry a UTC offset")
    return parsed
...
def _sha(value: Any) -> str:
    if not isinstance(value, str) or len(value) != 64:
        raise ValueError("T20.43 expected a SHA-256 hex digest")
    try:
        int(value, 16)
    except ValueError as error:
        raise ValueError("T20.43 expected a SHA-256 hex digest") from error
    return value


def _commit(value: Any) -> str:
    if not isinstance(value, str) or len(value) != 40:
        raise ValueError("T20.43 expected a full Git commit")
    try:
        int(value, 16)
    except ValueError as error:
        raise ValueError("T20.43 expected a full Git commit") from error
    return value


def _finite(value: Any, *, label: str) -> float:
    if isinstance(value, bool) or not isinstance(value, (int, float)):
        raise ValueError(f"T20.43 {label} must be numeric")
    number = float(value)
    if not math.isfinite(number) or number < 0.0:
        raise ValueError(f"T20.43 {label} must be finite and nonnegative")
    return number
