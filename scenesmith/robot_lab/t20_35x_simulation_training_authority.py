"""Central training-only authority for the T20.35x physical-gate joint-weighted correction."""
...
OWNER_GRANT_PATH = Path(
    "configurations/robot_lab/t20_35x_owner_training_authorization.json"
)
REQUEST_PATH = Path(
    "configurations/robot_lab/t20_35x_simulation_training_authority_request.json"
)
DECISION_PATH = Path(
    "configurations/robot_lab/t20_35x_simulation_training_authority_decision.json"
)
VALID_FROM = "2026-07-15T07:27:47-05:00"
VALID_UNTIL = "2026-07-15T23:27:47-05:00"
EVALUATION_TIME = "2026-07-15T16:05:00-05:00"
AUTHORIZED_ACTIONS = (
    ...
    return sign_payload(
        {
            ...
            "authorization_id": "t20_35x_physical_gate_joint_weighted_consistency_grant",
            "subject_id": DEFAULT_AUTHORITY_SUBJECT_ID,
            "scope_id": DEFAULT_AUTHORITY_SCOPE_ID,
            "authorization_scope": "t20_35x_simulation_only_physical_gate_joint_weighted_correction",
...
def verify_owner_grant(
    ...
    *,
    ...
    verify_signed_payload(payload, label="T20.35x owner training grant")
    if payload != build_owner_grant(training_spec=training_spec, repo_root=repo_root):
        raise ValueError("T20.35x owner grant drifted")
...
def build_production_authority(
    *, repo_root: Path = REPO_ROOT
    ...
    trajectory = load_strict_json(root / T20_35W_AUDIT_PATH)
    verify_signed_payload(trajectory, label="T20.35x physical-gate evidence")
    if trajectory.get("identity_sha256") != EXPECTED_T20_35W_AUDIT_IDENTITY:
        raise ValueError("T20.35x physical-gate authority source drifted")
    dataset = load_strict_json(root / DATASET_MANIFEST_PATH)
    verify_signed_payload(dataset, label="T20.35x dataset manifest")
    ...
    refs = {
        ...
        "trajectory": artifact_ref(
            path=T20_35W_AUDIT_PATH, payload=trajectory, repo_root=root
        ),
    ...
    for prerequisite_id, evidence_refs in sorted(evidence.items()):
        ...
        claims.append(
            build_capability_claim(
                claim_id=f"t20_35x_{prerequisite_id}",
                ...
                evidence_refs=[
                    build_evidence_ref(
                        artifact_kind="t20_35x_physical_gate_joint_weighted_training_evidence",
    ...
    if decision.get("authority_granted") != ["simulation_training_ready"]:
        raise ValueError("T20.35x central authority exceeded training-only scope")
...
def verify_authority(*, repo_root: Path = REPO_ROOT) -> dict[str, Any]:
    ...
    if request != expected_request:
        raise ValueError("T20.35x authority request drifted")
    verify_authority_decision(decision, request=request)
    if decision != expected_decision:
        raise ValueError("T20.35x authority decision drifted")
...
def require_active_authority(
    *, repo_root: Path = REPO_ROOT, now: datetime | None = None
    ...
    if current < datetime.fromisoformat(VALID_FROM):
        raise ValueError("T20.35x authority is not active yet")
    if current > datetime.fromisoformat(VALID_UNTIL):
        raise ValueError("T20.35x authority has expired")
    return result
