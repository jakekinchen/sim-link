"""Central simulation-only authority for the exact T20.36e ACT control."""
...
OWNER_GRANT_PATH = Path(
    "configurations/robot_lab/t20_36e_owner_training_authorization.json"
)
REQUEST_PATH = Path(
    "configurations/robot_lab/t20_36e_simulation_training_authority_request.json"
)
DECISION_PATH = Path(
    "configurations/robot_lab/t20_36e_simulation_training_authority_decision.json"
)
VALID_FROM = "2026-07-15T07:27:47-05:00"
VALID_UNTIL = "2026-07-15T23:27:47-05:00"
EVALUATION_TIME = "2026-07-15T19:04:00-05:00"
AUTHORIZED_ACTIONS = (
    ...
    return sign_payload(
        {
            ...
            "authorization_id": "t20_36e_exact_act_gate_b_control_grant",
            "subject_id": DEFAULT_AUTHORITY_SUBJECT_ID,
            "scope_id": DEFAULT_AUTHORITY_SCOPE_ID,
            "authorization_scope": (
                "one_exact_act_one_batch_simulation_only_gate_b_diagnostic_control"
            ),
            ...
            "authorization_source": (
                "owner_resumed_goal_loop_and_pre_registered_"
                "post_x_act_control_2026_07_15"
            ),
        }
    )
...
def verify_owner_grant(
    ...
    *,
    ...
    verify_signed_payload(payload, label="T20.36e owner training grant")
    if payload != build_owner_grant(
        training_spec=training_spec, repo_root=repo_root
    ):
        raise ValueError("T20.36e owner grant drifted")
...
def build_production_authority(
    *, repo_root: Path = REPO_ROOT
    ...
    verify_signed_payload(local_preflight, label="T20.36e local policy preflight")
    verify_signed_payload(dataset, label="T20.36e canonical dataset manifest")
    ...
    evidence = {
        ...
        "structural_contract_valid": [refs["dataset_authority"], refs["spec"]],
        "executable_stack_valid": [refs["preflight"], refs["spec"]],
    ...
    for prerequisite_id, evidence_refs in sorted(evidence.items()):
        ...
        claims.append(
            build_capability_claim(
                claim_id=f"t20_36e_{prerequisite_id}",
                ...
                evidence_refs=[
                    build_evidence_ref(
                        artifact_kind="t20_36e_exact_act_control_evidence",
    ...
    if decision.get("authority_granted") != ["simulation_training_ready"]:
        raise ValueError("T20.36e central authority exceeded training-only scope")
...
def verify_authority(*, repo_root: Path = REPO_ROOT) -> dict[str, Any]:
    ...
    if request != expected_request:
        raise ValueError("T20.36e authority request drifted")
    verify_authority_decision(decision, request=request)
    if decision != expected_decision:
        raise ValueError("T20.36e authority decision drifted")
...
def require_active_authority(
    *, repo_root: Path = REPO_ROOT, now: datetime | None = None
    ...
    if current.tzinfo is None or current.utcoffset() is None:
        raise ValueError("T20.36e authority time must carry a UTC offset")
    if current < datetime.fromisoformat(VALID_FROM):
        raise ValueError("T20.36e authority is not active yet")
    if current > datetime.fromisoformat(VALID_UNTIL):
        raise ValueError("T20.36e authority has expired")
    return result
