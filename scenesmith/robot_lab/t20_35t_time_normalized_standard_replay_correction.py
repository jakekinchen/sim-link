"""Exact-state time-normalized standard-replay correction for T20.35t."""
...
from scenesmith.robot_lab.t20_35s_objective_mass_compatibility_audit import (
    AUDIT_PATH as T20_35S_AUDIT_PATH,
    load_source_artifacts as load_t20_35s_sources,
    verify_audit as verify_t20_35s_audit,
)


REPO_ROOT = Path(__file__).resolve().parents[2]
SPEC_PATH = Path(
    "configurations/robot_lab/t20_35t_time_normalized_standard_replay_training_spec.json"
)
RESULT_PATH = Path(
    "configurations/robot_lab/t20_35t_time_normalized_standard_replay_result.json"
)
SCHEMA_VERSION = "scenesmith.t20_35t_time_normalized_standard_replay_training_spec.v1"
RUN_SCHEMA_VERSION = "scenesmith.t20_35t_time_normalized_standard_replay_run.v1"
RESULT_SCHEMA_VERSION = "scenesmith.t20_35t_time_normalized_standard_replay_result.v1"
...
EXPECTED_T20_35S_AUDIT_IDENTITY = (
    "113f72a96f8650fcfe6f3b84a51312fed7d0eb62c74d22a0162111ea043a92db"
)
...
STANDARD_REPLAY_SEED_BASE = 20261717
...
def build_correction_examples(
    ...
    verify_signed_payload(trajectory_result, label="T20.35t trajectory source")
    verify_signed_payload(target_result, label="T20.35t normalized-target source")
    if (
        ...
        raise ValueError("T20.35t trajectory route drifted")
    ...
    if _matrix_sha(target) != target_result.get(
        ...
        raise ValueError("T20.35t normalized target hash drifted")
    ...
    for row in rows:
        ...
        for step_index in CORRECTION_STEP_INDICES:
            ...
            if _matrix_sha(state) != step.get("state_sha256"):
                raise ValueError("T20.35t correction state hash drifted")
            reference = [
                [
            ...
            if (
                ...
                raise ValueError("T20.35t exact-state reconstruction exceeded tolerance")
            examples.append(
                {
    ...
    if len(examples) != CORRECTION_EXAMPLE_COUNT:
        raise ValueError("T20.35t correction example count drifted")
    return examples
...
def materialize_correction_tensors(
    ...
    for manifest_row, trajectory in zip(
        ...
        if _matrix_sha(noise) != manifest_row["derived_noise_sha256"]:
            raise ValueError("T20.35t materialized correction noise drifted")
...
def build_training_spec(
    *,
    ...
    trajectory_result: dict[str, Any],
    target_result: dict[str, Any],
    objective_mass_audit: dict[str, Any],
) -> dict[str, Any]:
    for label, payload in (
        ...
        ("objective-mass audit", objective_mass_audit),
    ):
        verify_signed_payload(payload, label=f"T20.35t {label}")
    if (
        ...
        or objective_mass_audit.get("identity_sha256")
        != EXPECTED_T20_35S_AUDIT_IDENTITY
        or objective_mass_audit.get("objective_compatibility_classification")
        != "terminal_objective_mass_dominance_with_standard_interference"
        or objective_mass_audit.get("selected_next_hypothesis")
        != "design_time_normalized_standard_replay_correction"
        or objective_mass_audit.get("gate_b_passed") is not False
    ):
        raise ValueError("T20.35t source identity or route drifted")
    ...
    if [sample_order.count(index) for index in range(CORRECTION_EXAMPLE_COUNT)] != [
        ...
        raise ValueError("T20.35t correction schedule is not balanced")
    time_weights = _derive_time_weights(objective_mass_audit)
    standard_seeds = [
    ...
    return sign_payload(
        {
            ...
            "task_id": "T20.35t",
            "scope": "one_bounded_expert_only_time_normalized_full_path_correction_with_standard_replay",
            ...
            "t20_35s_audit_identity_sha256": objective_mass_audit[
                "identity_sha256"
            ],
            ...
            "time_normalization": {
                "formula": "global_baseline_correction_mean_divided_by_step_baseline_mean",
                "source_baseline_correction_objective_mean": sum(
                    row["baseline_objective_mean"]
                    for row in objective_mass_audit["objective_by_step"]
                )
                / len(CORRECTION_STEP_INDICES),
                "weight_by_step": time_weights,
                "correction_loss_coefficient": 1.0,
            ...
            "campaign": {
                ...
                "gradient_accumulation": "one_weighted_correction_plus_one_standard_replay_before_each_step",
...
def verify_training_spec(
    ...
    *,
    ...
    verify_signed_payload(payload, label="T20.35t training spec")
    expected = build_training_spec(
        ...
        objective_mass_audit=objective_mass_audit,
    )
    if payload != expected:
        raise ValueError("T20.35t training spec drifted")


def verify_run(
    payload: dict[str, Any], *, spec: dict[str, Any], authority_identity: str
) -> None:
    verify_signed_payload(payload, label="T20.35t run")
    ...
    if (
        ...
        raise ValueError("T20.35t run campaign or lineage drifted")
    for field in (
    ...
    raw_updates = payload.get("per_update_raw_correction_objective")
    weighted_updates = payload.get("per_update_weighted_correction_objective")
    standard_updates = payload.get("per_update_standard_replay_objective")
    ...
    baseline_weighted = payload.get("baseline_weighted_objective_by_example")
    final_weighted = payload.get("final_weighted_objective_by_example")
    ...
    if (
        ...
        or len(raw_updates) != OPTIMIZER_UPDATES
        or not isinstance(weighted_updates, list)
        or len(weighted_updates) != OPTIMIZER_UPDATES
        or not isinstance(standard_updates, list)
        or len(standard_updates) != OPTIMIZER_UPDATES
        ...
        or not isinstance(baseline_weighted, list)
        or len(baseline_weighted) != CORRECTION_EXAMPLE_COUNT
        or not isinstance(final_weighted, list)
        or len(final_weighted) != CORRECTION_EXAMPLE_COUNT
        ...
        raise ValueError("T20.35t objective or gradient trace is incomplete")
    for value in (
    ...
    if payload["baseline_correction_objective_mean"] <= 0.0:
        raise ValueError("T20.35t baseline correction objective must be positive")
    weights = spec["time_normalization"]["weight_by_step"]
    for update, example_index in enumerate(campaign["sample_index_by_update"]):
        expected_weighted = raw_updates[update] * weights[example_index % 10]
        if (
            abs(weighted_updates[update] - expected_weighted) > 1e-6
            or abs(losses[update] - weighted_updates[update] - standard_updates[update])
            > 1e-6
        ):
            raise ValueError("T20.35t paired update objective drifted")
    for index, (raw_before, raw_after) in enumerate(
        zip(baseline_by_example, final_by_example, strict=True)
    ):
        weight = weights[index % 10]
        if (
            abs(baseline_weighted[index] - raw_before * weight) > 1e-6
            or abs(final_weighted[index] - raw_after * weight) > 1e-6
        ):
            raise ValueError("T20.35t weighted correction evidence drifted")
    if (
        abs(
        ...
        > 1e-12
        or abs(
            sum(baseline_weighted) / CORRECTION_EXAMPLE_COUNT
            - payload["baseline_weighted_correction_objective_mean"]
        )
        > 1e-12
        or abs(
            sum(final_weighted) / CORRECTION_EXAMPLE_COUNT
            - payload["final_weighted_correction_objective_mean"]
        )
        > 1e-12
        or abs(
        ...
        raise ValueError("T20.35t correction objective mean drifted")
    if (
        abs(
            payload["baseline_standard_objective_mean"]
        ...
        raise ValueError("T20.35t source checkpoint standard objective drifted")
    for field in ("trainable_parameter_count", "paligemma_parameter_count"):
        if (
            ...
            raise ValueError(f"T20.35t {field} is invalid")
    ...
    if (
        ...
        raise ValueError("T20.35t decoded seed coverage drifted")
    for row in chunks:
    ...
    if not isinstance(tree, list) or len(tree) < 2:
        raise ValueError("T20.35t checkpoint tree is incomplete")
    if hashlib.sha256(canonical_json_bytes(tree)).hexdigest() != payload[
        "checkpoint_identity_sha256"
    ]:
        raise ValueError("T20.35t checkpoint identity drifted")
    required_false = (
        "checkpoint_mutated",
        "dataset_mutated",
        "statistics_changed",
    ...
    if any(payload.get(field) is not False for field in required_false):
        raise ValueError("T20.35t run authority fields drifted")


def build_result(
    *, spec: dict[str, Any], authority_identity: str, run: dict[str, Any]
) -> dict[str, Any]:
    verify_signed_payload(spec, label="T20.35t result spec")
    ...
    weighted_baseline = _finite(
        run["baseline_weighted_correction_objective_mean"],
        "baseline weighted correction objective",
    )
    weighted_final = _finite(
        run["final_weighted_correction_objective_mean"],
        "final weighted correction objective",
    )
    weighted_ratio = weighted_final / weighted_baseline
    standard_ratio = (
    ...
    if gate_b_passed:
        ...
        route = "balanced_objectives_pass_action_fail_route_post_training_path_audit"
    elif objective_pass:
        route = "standard_objective_pass_correction_objective_fail_route_correction_attainability_audit"
    elif action_pass:
        route = "action_pass_balanced_objective_fail_route_objective_floor_audit"
    else:
        route = "balanced_correction_both_fail_route_gradient_compatibility_audit"
    return sign_payload(
        {
            ...
            "t20_35s_audit_identity_sha256": spec["t20_35s_audit_identity_sha256"],
            ...
            "baseline_weighted_correction_objective_mean": weighted_baseline,
            "final_weighted_correction_objective_mean": weighted_final,
            "final_to_baseline_weighted_correction_objective_ratio": weighted_ratio,
...
def verify_result(
    ...
    verify_signed_payload(payload, label="T20.35t result")
    ...
    if not _equal_with_float_tolerance(archived, rebuilt, absolute_tolerance=1e-15):
        raise ValueError("T20.35t result drifted")
...
def load_source_artifacts(*, repo_root: Path = REPO_ROOT) -> dict[str, Any]:
    ...
    audit_sources = load_t20_35s_sources(repo_root=root)
    objective_mass_audit = load_strict_json(root / T20_35S_AUDIT_PATH)
    verify_t20_35s_audit(objective_mass_audit, **audit_sources)
    return {
        **trajectory_sources,
...
def _finite_matrix(
    ...
    if not isinstance(value, list) or len(value) != rows:
        raise ValueError(f"T20.35t {label} row count drifted")
    matrix = []
    for row in value:
        if not isinstance(row, list) or len(row) != columns:
            raise ValueError(f"T20.35t {label} column count drifted")
        matrix.append([_finite(item, label) for item in row])
    return matrix


def _derive_time_weights(audit: dict[str, Any]) -> list[float]:
    rows = audit.get("objective_by_step")
    if (
        not isinstance(rows, list)
        or len(rows) != len(CORRECTION_STEP_INDICES)
        or [row.get("step_index") for row in rows if isinstance(row, dict)]
        != list(CORRECTION_STEP_INDICES)
    ):
        raise ValueError("T20.35t objective-mass step coverage drifted")
    means = [
        _finite(row.get("baseline_objective_mean"), "step baseline objective")
        for row in rows
    ]
    if any(value <= 0.0 for value in means):
        raise ValueError("T20.35t step baseline objective must be positive")
    global_mean = sum(means) / len(means)
    weights = [global_mean / value for value in means]
    if any(abs(weight * value - global_mean) > 1e-12 for weight, value in zip(weights, means, strict=True)):
        raise ValueError("T20.35t time normalization failed exact balance")
    return weights


def _finite(value: Any, label: str) -> float:
    if isinstance(value, bool) or not isinstance(value, (int, float)):
        raise ValueError(f"T20.35t {label} must be finite")
    number = float(value)
...
def _sha(value: Any, label: str) -> None:
    if (
        ...
        raise ValueError(f"T20.35t {label} must be lowercase SHA-256")
